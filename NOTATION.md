...
citing the use of `\beta` for a coalgebra structure map in `sec.pc`. That was
...
| Integrator | `\intg` (renders `\mathfrak{i}`) | the pair `\intg=(\Fun S,\dyn)` (`def.integrator`). Instances: `\intg_{\mathrm{conf}}` (configuration, `\Fun S=\absval\blank`) and `\intg_{\mathrm{phase}}` (phase, `\Fun S=\absval{T^*\blank}`) |
| Dynamics | `\dyn` (renders `\mathrm{dyn}`) | the second component of an integrator: a monoidal nat. trans. `\Store\circ\Fun S\Rightarrow p`. Instances: `\chi` (the Euler step; `\chiQ` for its `\rv Q`-component, distinct from the lax/colax `\theta` of line 213; the configuration dynamics is `(-1)\circ\chi`), `\dynphase` (phase; renders `\mathrm{dyn}_{\mathrm{phase}}`). The `\Para_{\cat Q}^\dyn` functors in `prop.integrator_to_pc` carry the dynamics as superscript (the action-square datum) and the acting category `\cat Q` as subscript (the identity `F`, per the suppression convention after `prop.para_square`) |
| Kinetic update | `\nu`, `\nu_{\rho,\omega}` | monoidal nat. trans. `\cot\circ T^*\Rightarrow\cot` assembled from a readout `\rho` and a 1-form `\omega` (`lem.combined_update`): reads out `\rho`, injects `(\xi',0)+\omega`. The phase integrator uses `\nu=\nu_{\exp,\beta}` (presented position + kinetic), injecting `(\xi',\sharpR_q\xi)`; `\dynphase=\nu\circ\chiQ{T^*\blank}` (`sec.phase_integrators`). Greek `\nu`, distinct from `\dyn` |
| Monoidal readout | `\rho`; `\pi`, `\exp`, `\exp^\alpha` | natural monoidal `T^*Q\to Q` (`def.monoidal_readout`); generic readout `\rho`; `\pi=\exp^0` projection, `\exp=\exp^1` exponential (= presented position), `\exp^\alpha` the time-`\alpha` exponential interpolating them; they form a vector space (`prop.readouts_vector_space`). Named instances use `\exp` (not `\rho^\alpha`) so the geometer-standard time-`\alpha` reading is clear. `\rho` is unused elsewhere—no clash |
| Monoidal 1-form | `\omega`; `\beta`, `\zeta` | natural monoidal section `T^*Q\to T^*(T^*Q)` (`def.monoidal_one_form`); `\beta` kinetic (`ex.kinetic_one_form`), `\zeta` dissipation (`ex.dissipation_one_form`); a vector space (`prop.one_forms_monoid`). **Clash:** `\omega_M,\omega_N` are covector-field coordinates in `ch.smooth_dynamics` |
...
| Integrator semantics | `\Psisem` (renders `\Psi`) | `\Psi_\intg\colon\para p\poly\to\pciso` (`prop.integrator_to_pc`; included into `\pc` by `thm.dynamics_functor`); indexed by the integrator pair |
...
labels keep the legacy `rewiring` stem; the `org` label stem was retired with the `\pc` rename, so labels now read `sec.pc`, `prop.pc_as_para`, `lem.poly_to_pc`, `prop.integrator_to_pc`; prose is "adaptive-arrangement" throughout), introduced in
