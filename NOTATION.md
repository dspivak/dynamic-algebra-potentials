...
| Kinetic update | `\nu`, `\nu_{\sigma,\omega}` | monoidal nat. trans. `\cot\circ T^*\Rightarrow\cot` assembled from a readout `\sigma` and a 1-form `\omega` (`lem.combined_update`): reads out `\sigma`, injects `(\xi',0)+\omega`. The phase integrator uses `\nu=\nu_{\sigma^1,\beta}` (presented position + kinetic), injecting `(\xi',\sharpR_q\xi)`; `\dynphase=\nu\circ\chiQ{T^*\blank}` (`sec.phase_integrators`). Greek `\nu`, distinct from `\dyn` |
| Monoidal readout | `\sigma`; `\pi`, `\sigma^1`, `\sigma^\alpha` | natural monoidal `T^*Q\to Q` (`def.monoidal_readout`); `\pi=\sigma^0` projection, `\sigma^1` exponential (= presented position), `\sigma^\alpha` interpolates; they form a vector space (`prop.readouts_vector_space`). **Clash:** `\sigma` is also the monad strength (row below)—disambiguate by context (phase semantics vs `ch.lenses`) |
