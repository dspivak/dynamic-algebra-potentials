...
| Kinetic update | `\nu` | the monoidal nat. trans. `\cot\circ T^*\Rightarrow\cot` that reads out the presented position `\tilde q` and injects `(\xi',\sharpR_q\xi)`; the phase dynamics factors as `\dynphase=\nu\circ\chiQ{T^*\blank}` (`sec.phase_integrators`). Greek `\nu`, distinct from `\dyn` (which renders `\mathrm{dyn}`) |
| Drift / kick | `\fun{drift}`, `\fun{kick}` | the two shears on `T^*Q` (`eqn.drift_kick`); each an `\rvect`-endomorphism of `T^*Q`, composite = closed quadratic update (`prop.drift_kick_reactive`) |
| Stiffness | `\kappa` | symmetric linear map `Q\to Q^*`, the `dU` of the quadratic potential `U(q)=\tfrac12(\kappa q)(q)` (block before `prop.drift_kick_reactive`); the scalar `\kappa` of `sec.wave_equation` is its 1-dimensional instance |
