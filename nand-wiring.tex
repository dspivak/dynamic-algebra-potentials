...
$\fun{node}$ ($V_{\!out}$, $V_{\!mid}$)
  & $\lensunit\to\binom{\rr^0}{\rr}$ & $\rr$ & $c$ & $0$\\[.4ex]
$\fun{rail}$
...
\textbf{Reporters.} $\outp f(q)=q$, and the only covector reaching the state is the total current drawn on
the node, so the step is $q\mapsto q-c\,\xi$. Taking $\sharpR=0$ freezes the state: that is the ideal
voltage source.
