...
For any paired vector space $(V,\sharp_V)$, $T^*V\cong V\oplus V^*$ by \eqref{eqn.tangent_vec}. We write $(v,\pi)$ for a \emph{position} $v\in V$ and \emph{momentum} $\pi\in V^*$. The cotangent space decomposes as
\[
T^*_{(v,\pi)}(T^*V)\cong T^*_vV\oplus T^*_\pi(V^*)\cong V^*\oplus V,
\]
again by \eqref{eqn.tangent_vec}, using $(V^*)^*\cong V$ on the $V^*$-factor.

\begin{definition}[Pairing projection]\label{def.pairing_proj}
The \emph{pairing projection} $\rho_V\colon\cot{T^*V}\to\cot{V}$ has position-action $(v,\pi)\mapsto v$ and direction-action at $(v,\pi)$
\[
T^*_vV\cong V^*\To{\xi_V\mapsto(\xi_V,\,\sharp_V(\pi))}V^*\oplus V\cong T^*_{(v,\pi)}(T^*V).
\qedhere
\]
\end{definition}
...
\]
which at $v\in V$ sends a point $m\in M$ to  $n\coloneqq f(v,m)\in N$ and for a covector $\xi_N\in T^*_nN$ if we let $(\xi_V,\xi_M)\coloneqq(Df)_{(v,m)}^\top(\xi_N)\in T_v^*V\times T_m^*M$, returns $\xi_M\in T^*_mM$ and updates to
...
\end{proposition}
\begin{proof}
This is an immediate corollary of the \cref{prop.Theta_z} with $\Fun{T}=\id$. 
...
The $(\cot{\rr}\otimes\blank)$-monoid structures $(z,e,m,\alpha)$ for which $z=\yon$ are in bijection with real numbers $d:\rr$.
\end{lemma}
...
\]
Setting $x_2\coloneqq 0$, we find $\omega(0)=\omega(x_1)$ for all $x_1$. In other words, $\omega=d$ is constant at some $d:\rr$. And conversely, any constant covector field satisfies the condition.
 \end{proof}
...
\dec\colon\cot{\rr}\to\yon,\qquad\bk\dec(x)\coloneqq -1\in T^*_x\rr
\end{equation}
...
\[
\para{\pvect}{\lmfd^\rr}\to\para{\cot}{\poly}.
\]
\end{lemma}

\begin{proof}
We apply \cref{prop.para_square} to $\Lens\cot$ from \cref{lem.cot_lifts_lens_potential} and apply \cref{cor.parameterized_representation} to $\Theta_\dec\colon\Lens{\poly}^\rr\to\poly$ from
\cref{lem.Theta_poly_potential} to obtain the maps below; the desired functor is their composite:
\[
\para{\pvect}{\lmfd^\rr}\To{\para\pvect{\Lens\cot}}
\para{\pvect}{\Lens\poly^\rr}\To{\para\pvect{\Theta_\dec}}
\para{\cot}{\poly}.
\qedhere
\]
\end{proof}

\begin{lemma}\label{lem.rho_star}
...
\[
\rho^*\colon\para{\cot}{\poly}\to\para{\cot{T^*}}{\poly}.
\]
\end{lemma}
...
\[
\Phi\colon\para{\pvect}{\Lens{\mfd}^{\rr}}\to\org.
\]
\end{theorem}

\begin{proof}
Define $\Phi$ to be the composite of the maps below, given by \cref{lem.potlens_to_para_poly}, \cref{lem.rho_star}, and \cref{lem.poly_to_org}:
\[
\para{\pvect}{\lmfd^\rr}\To{}
\para{\cot}{\poly}\To{\rho^*}
\para{\cot{T^*}}{\poly}\To{}
\org.
\qedhere
\]
...
We now unpack $\Phi$. Let $\inpt M,\outp M:\mfd$ be manifolds, and write
\[\Omega(M)\coloneqq\{\omega\colon M\to T^*M\mid\pi\circ\omega=\id\}\]
for the set of (not-necessarily continuous) \emph{covector fields} on $M$. A smooth map $f\colon M\to N$ induces a \emph{pullback} $f^*\colon\Omega(N)\to\Omega(M)$ defined by $(f^*\omega)(m)\coloneqq(Tf_m)^\top\omega(f(m))$.

The action of $\Phi$ on objects is given as follows.
\[
\Phi\lensob M=
...
For the action of $\Phi$ on morphisms, let $(V,\sharp):\pvect$ and let
$f\colon V\cdot\lensob M\to\lensob N$
be a morphism in $\para{\pvect}{\lmfd^\rr}$, i.e.\ a tuple $(\outp f,\inpt f,U)$:
...
\]
writing $x=(v,\pi)\in X$ for position $v\in V$ and momentum $\pi\in V^*$. 
...
\]
Given a state $x\coloneqq(v,\pi)\in X$ and a position $a\coloneqq(\outp m,\omega):\outp M\times\Omega(\inpt M)$, the readout position is
\[
b\coloneqq\bigl(\outp f(v,\outp m),\,\omega'\bigr)\;\in\;\outp N\times\Omega(\inpt N),
\]
where the output covector field $\omega'\colon\inpt N\to T^*\inpt N$ is
\begin{equation}\label{eqn.omegaprime}
\omega'(\inpt n)\;=\;(\inpt f(v,\outp m,\blank))^*\omega\big|_{\inpt n}\;-\;d(U(v,\outp m,\blank))\big|_{\inpt n}.
\end{equation}
The first term is the pullback of $\omega$ along the partial map $\inpt f(v,\outp m,\blank)\colon\inpt N\to\inpt M$; the second is the differential of the potential $U(v,\outp m,\blank)\colon\inpt N\to\rr$ at $\inpt n$.%
\footnote{
The formula \eqref{eqn.omegaprime} comes from tracing through \cref{lem.cot_lifts_lens_potential,lem.Theta_poly_potential}: in $\Lens{\mfd}^\rr$, the lens backward is the augmented map
\[(v,\outp m,\inpt n)\mapsto(U(v,\outp m,\inpt n),\,\inpt f(v,\outp m,\inpt n))\colon V\times\outp M\times\inpt N\to\rr\times\inpt M.\]
Applying $\cot$ takes its Jacobian transpose, which sends $(r,\xi)\in T^*_{U}\rr\times T^*_{\inpt m}\inpt M$ to the covector at $(v,\outp m,\inpt n)$ whose $\inpt N$-component is
\[r\cdot\partial_{\inpt N}U(v,\outp m,\inpt n) \;+\; \xi\circ\partial_{\inpt N}\inpt f(v,\outp m,\inpt n).\]
We substitute $(r,\xi)\coloneqq(-1,\omega(\inpt m))$ by \eqref{eqn.dec}. Together these give $-d(U(v,\outp m,\blank))\big|_{\inpt n} + ((\inpt f(v,\outp m,\blank))^*\omega)\big|_{\inpt n}$, which is \eqref{eqn.omegaprime}.
}

Now given an input direction $(\xi_N,\inpt n)\in\Phi\lensob N[b]$, the coalgebra $\Phi(f)$ at $(x,a)$ produces a source direction by reading off components of a single 1-form on $V\times\outp M\times\inpt N$. Let $\inpt m\coloneqq\inpt f(v,\outp m,\inpt n)$ and define
\begin{equation}\label{eqn.bigtheta}
\Theta\coloneqq\bigl((\outp f)^*\xi_N + (\inpt f)^*\omega - dU\bigr)\big|_{(v,\outp m,\inpt n)}\;\in\; V^*\oplus T^*_{\outp m}\outp M\oplus T^*_{\inpt n}\inpt N,
\end{equation}
where $(\outp f)^*\xi_N$ pulls the covector $\xi_N\in T^*_{\outp f(v,\outp m)}\outp N$ back along $\outp f$ (extended trivially on the $\inpt N$-factor), $(\inpt f)^*\omega$ is the field-level pullback of $\omega$ along $\inpt f$, and $dU$ is the differential of $U$. Decompose $\Theta=(\Theta_V,\,\Theta_{\outp M},\,\Theta_{\inpt N})$. Then \eqref{eqn.omegaprime} reads $\omega'(\inpt n)=\Theta_{\inpt N}$, and the coalgebra at $(x,a)$ on $(\xi_N,\inpt n)$ returns the direction
\[
\xi_M\coloneqq(\Theta_{\outp M},\,\inpt m)\in\Phi\lensob M[a]
\]
and updates the state to
\[
x+\sharp_X(\Theta_V,\sharp_V(\pi))=\bigl(v+\sharp_V(\pi),\,\pi-\Theta_V\bigr),
\]
where the lift $(\Theta_V,\sharp_V(\pi))\in T^*_xX$ comes from \cref{def.pairing_proj}.
\Cref{prop.coalg_as_poly_map,prop.pnla_polynomial,lem.cot_lifts_lens_potential,lem.Theta_poly_potential,def.pairing_proj}.

\section{Example: the spring cell}\label{sec.spring}
...
At state $(x,p): T^*V$ on input $(y,\omega)$ with vanishing received covector $\xi=0$ and input direction $(\ell,r)\in \rr^2$, the update of $\Phi(f_\mathrm{spring})$ is
\[
\dot x = p/\mu,
\qquad
...
\begin{proof}
Unwinding $\Phi(f_\mathrm{spring})$ at $(x,p)$: the $\sharp_V(p)=p/\mu$ component of $\dot x$ is the $V$-slot of the pairing projection (\cref{def.pairing_proj}). The $\dot p$ component receives contributions from $U$, $\outp f$, and $\inpt f$ along the chain induced by $\rho_V$; only the $U$-contribution is nonzero here ($\partial_V\inpt f=0$ and $\xi=0$), giving $\dot p=-\partial_xU=\kappa(\ell+r-2x)$.\qedhere
\end{proof}
