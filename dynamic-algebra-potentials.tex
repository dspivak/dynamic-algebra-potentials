...
To each $(\vec v,\vec \omega)$, the vector of outputs of the internal boxes, we assign the external output $(v_K,\omega_1)$ (the rightmost particle's position with the leftmost box's covector field); and to each external input $(\xi_N,v_0)$ at the big box, we assign the internal inputs $(0,v_0)$, $\ldots$, $(0,v_{K-2})$, $(\xi_N,v_{K-1}))$---the rightmost box gets the external $\xi_N$ in its $\xi$-slot, the others get $0$---and each box receives its left neighbor's position. In our concise format, 
\[
(\vec v,\vec \omega)\mapsto\Big((v_K,\omega_1),\;(\xi_N,v_0)\mapsto\bigl((0,\ldots,0,\xi_N),\,(v_0,v_1,\ldots,v_{K-1})\bigr)\Big).
\]
All the zeros indicate that the wiring itself is not applying any force to the little boxes, other than handing the outer $v_0$ position to the first box and the covector $\xi_N$ to the last box in \eqref{eqn.wave_wd}.
 
...
At state $(v,p)\in T^*V$ on a position $a=(\outp m,\omega)$ with $\omega=0$ and input direction $(\xi_N,\inpt n)$ with $\xi_N=0$, the coalgebra $\Phi(f_\mathrm{spring})$ updates the state to
\[
\dot v = p/\mu,
\qquad
