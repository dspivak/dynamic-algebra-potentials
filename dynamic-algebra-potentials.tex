...
\end{proposition}
\begin{proof}
Given a coKleisli morphism $f\colon\lensob{c}\coto\lensob{d}$, define $\Theta_z(f)$ as the composite
\[
...
For composition, the obligation is the equality of two morphisms $\outp c\,H_c\to\outp e\,H_e$ (writing $H_X\coloneqq[\inpt X,z]$): $\Theta_z$ applied to the coKleisli composite $g\circ f$ vs the sequential $\Theta_z(g)\circ\Theta_z(f)$. The two atomized boundaries (each arrow performs exactly one nonidentity operation, in bold; juxtaposition denotes $\otimes$; $\sigma$ has been opened as $\eta$ then $\tau$) are:

The top--right boundary unfolds $\Theta_z(g)\circ\Theta_z(f)$: the f-half (atoms $\delta,\coev_d,\outp f,\eta,\inpt f,\tau,\Fun T\ev,\delta$) produces $H_d$, the bridge $(\eta, \Fun T\alpha)$ applies $\eta$-naturality at $\alpha$ to lift the intermediate $\alpha$ inside $\Fun T$, and the g-half ($\outp g,\coev_e,\inpt g,\tau,\Fun T\ev$) produces $H_e$. The left--bottom boundary unfolds $\Theta_z(g\circ f)$ directly via the Kleisli formula for $\inpt{(g\circ f)}$ involving $\Fun T\inpt f,\mu$.
...
The open hole splits into two rectangular regions along the equal arrow. \emph{Upper region} ($4\!\times\!4$, closes via comonoid-hom for $\outp f$); \emph{Lower region} ($5\!\times\!4$, with $[dd]$ and $[rr]$ on LB to absorb the atom-count gap):
...
The lax unit comparison is $e_z\colon I\to z=\Theta_z\binom{I}{I}$, and the lax laxator is the canonical map
\[
...
\]
on $\Lens{\cat C}^{\Fun T}$ and on $\cat C$ respectively, $\Theta_z\colon\Lens{\cat{C}}^\Fun{T}\to\cat{C}$  extends to a lax symmetric monoidal functor
\[
\para{\cat M}{\Theta_z}\colon\para{\cat M}{\Lens{\cat C}^{\Fun T}}\to\para{\cat M}{\cat C}.
\]
\end{proposition}
...
\begin{proof}
Use the action on $\Lens{\cat C}^{\Fun T}$ from \cref{lem.parameter_lens_action} and the action $m\cdot x\coloneqq J(m)\otimes x$ on $\cat C$. The functor $\Theta_z$ is $\cat M$-equivariant:
\[
\begin{tikzcd}[ampersand replacement=\&]
\cat M\times\Lens{\cat C}^{\Fun T}
  \ar[r,"(\cdot)"]
  \ar[d,"\id_{\cat M}\times\Theta_z"']
...
J(m)\otimes\Theta_z\lensob c
\cong J(m)\otimes\outp c\otimes\ihom{\inpt c,z}
=\Theta_z(m\cdot\lensob c)
\]
...
If $\cat{C}$ is a symmetric monoidal category and $(M,e,(*))$ is a commutative monoid object in it, then $M\otimes-\colon\cat{C}\to\cat{C}$ carries a monoidal monad structure.
\end{lemma}
...
\]
The coherence diagrams arise from the monoid laws and commutativity of $M$.
\end{proof}

Since $(\rr,0,+)$ is a commutative Lie group, \cref{prop.cot_hopf} gives a commutative monoid object $\cot{\rr}$ in $\poly$. By \cref{lemma.monoid_to_monad} there are corresponding monoidal monads $\rr\times-$ and $\cot{\rr}\otimes-$ on $\mfd$ and $\poly$, and by \cref{def.lens_cokleisli} there are corresponding comononads on $\Lens{\mfd}$ and $\Lens{\poly}$. We let $\Lens{\mfd}^\rr$ and $\Lens{\poly}^\rr$ denote the corresponding cokleisli categories.
...
The $(\cot{\rr}\otimes-)$-monoid structures $(z,e,m,\alpha)$ for which $z=\yon$ are in bijection with real numbers $d:\rr$.
\end{lemma}

\begin{proof}
There is a unique $\otimes$-monoid structure on $\yon$. Maps $\alpha\colon\cot{\rr}\to\yon$ are in bijection with (not-necessarily continuous) covector fields on $\rr$, i.e.\ sections $\bk{\alpha}\colon\rr\to T^*\rr$ of the projection $T^*\rr\to\rr$. The first condition of \eqref{eqn.T_monoid} is vacuous. Thus the only condition on $\alpha$ is that the following equation holds for all $x_1,x_2\in\rr$ 
\[
\left(\bk{\alpha}(x_1),\bk{\alpha}(x_2)\right)=
\left(\bk{\alpha}(x_1+x_2),\bk{\alpha}(x_1+x_2)\right)
\]
Setting $x_2\coloneqq 0$, we find $\bk{\alpha}(0)=\bk{\alpha}(x_1)$ for all $x_1$. In other words, $\bk{\alpha}=d$ is constant at some $d:\rr$. And conversely, any constant covector field satisfies the condition.
 \end{proof}

The role of the Lie group $\rr$ in this section is to act as the recipient of \emph{potentials}. The semantics of the value $a\in\rr$ is what potentials ``want to do,'' namely ``potentials want to decrease''. The formal representation of this is the choice 
\[
...
\[
\Theta_\dec\colon\Lens{\poly}^\rr\to\poly,\qquad
\Theta_\dec\lensob p\coloneqq\outp p\otimes\ihom{\inpt p,\yon}.
\]
%It sends a map $f\colon\lensob p\coto\lensob q$, i.e.\ $\outp f\colon \outp p\to\outp q$ and $\inpt f\colon\inpt q\otimes\outp p\to\cot{\rr}\otimes\inpt p$, to the composite
%\[
%\begin{tikzcd}[column sep=45pt, row sep=22pt, ampersand replacement=\&]
%\outp p\otimes\ihom{\inpt p,\yon}\ar[r,"{\delta_{\outp p}\otimes\id}"]\ar[dd, dashed, "\Theta_\dec(f)"']
...
%\& \outp q\otimes\ihom{\inpt q,\Fun T\yon}\ar[l,"{\id\otimes\ihom{\inpt q,\dec}}"]
...
\begin{proof}
This is \cref{lemma.monoid_to_monad,lem.alpha_constant,prop.Theta_T_alpha}.
\end{proof}
...
\begin{proof}
\cref{prop.cot_monoidal,lem.cot_comonoid} give $\cot\colon\pvect\to\poly$; \cref{lem.Theta_poly_potential} gives the lax monoidal $\Theta_\dec$; and \cref{prop.parameterized_representation} gives the induced functor.
\end{proof}
...
\[
J(V)\otimes\Theta_z\widehat F(\lensob L)\to\Theta_z\widehat F(\lensob M).
\]
\end{corollary}
...
The choice of $\dec$ fixes the sign of Hamilton's equations below; $c=0$ gives the trivial algebra (no potential), $c=+1$ the opposite-sign physics.
