...
The subsection extends to \emph{paired nilpotent Lie algebras} (PN Lie Algebras): pairs $(\lie g,\sharp)$ of a finite-dimensional nilpotent real Lie algebra $\lie g$ with a pairing on its underlying vector space, replacing ``vector space'' by ``nilpotent Lie algebra'' in \cref{def.pnla}. Direct sum remains monoidal (\cref{prop.pnla_monoidal}), since direct sums of nilpotent Lie algebras are nilpotent. Lie's third theorem~\cite[Appendix B]{knapp2002lie} attaches a simply connected Lie group $\Int\lie g$ to each such $\lie g$, and Malcev's theorem~\cite[Thm.~1.127]{knapp2002lie} makes $\exp_G\colon\lie g\To{\cong}\Int\lie g$ a diffeomorphism, so the action of \cref{prop.pnla_smc_action} reads $((\lie g,\sharp),M)\mapsto\Int\lie g\times M$.

Setting $G\coloneqq\Int\lie g$, the canonical symplectic pairing \eqref{eqn.canonical_form} of \cref{prop.canonical_symplectic_pairing} generalizes verbatim from $V\oplus V^*$ to $\lie g\oplus\lie g^*$, making $T^*G$ a PN Lie Algebra. The Lie groups $T^*G$ and $TG$ have, in left-trivialization, Lie algebras $\lie g\oplus\lie g^*$ and $\lie g\oplus\lie g$---the same underlying vector spaces as in \eqref{eqn.TT_def}---now with brackets mixing the $\ad$ and $\ad^*$ actions of $\lie g$; both are nilpotent of class at most $2n-1$ when $\lie g$ has class $n$, by a block-triangular argument on $\ad$. The morphism formulas \eqref{eqn.TT_phi} are unchanged, and \cref{prop.TT_endofunctors,prop.TT_monoidal} extend with the same swap isomorphisms \eqref{eqn.T_swap}, \eqref{eqn.Tstar_swap}; the only new ingredient is bracket-preservation in $T\phi$, $T^*\phi$, which uses the dual identity $(\phi^*)\inv\circ\ad^*_X=\ad^*_{\phi X}\circ(\phi^*)\inv$ obtained by transposing the Lie-algebra-homomorphism property of $\phi$. \Cref{prop.legendre} likewise extends, its components $\sharp_G\colon T^*G\to TG$ assembled from $\sharp_{\lie g}$ in left-trivialization. The exponential \eqref{eqn.flow_cot} extends from $T^*V\to V$ to $T^*G\to G$, $(g,\alpha)\mapsto g\cdot\exp_G(\sharp_G\alpha)$ in left-trivialization $T^*G\cong G\times\lie g^*$. The vector-space case treated above is abelian $\lie g$: $\Int\lie g=\lie g$ as Lie groups, $\exp_G=\id$, brackets collapse, and group multiplication is addition---so $(g,\alpha)\mapsto g\cdot\exp_G(\sharp_G\alpha)$ collapses to $(v,\alpha)\mapsto v+\sharp_V(\alpha)$, recovering \eqref{eqn.flow_cot}.\qedhere
\end{remark}
...
The assignment sending a paired vector space $(V,\sharp_V)$ to $V\yon^V$ extends to a functor $\pvect\to\poly$, and there is a natural isomorphism $V\yon^V\To{\cong}\cot{V}$ whose on-positions component is the identity and whose backward direction-component at each $v\in V$ is the cotangent flow \eqref{eqn.flow_cot} restricted to the fiber over $v$:
...
\begin{proof}
At each $v\in V$, \eqref{eqn.directions_sharp} is the composite of two linear isomorphisms with translation by $v$, hence a bijection; the on-positions map is the identity, also a bijection. So $V\yon^V\cong\cot{V}$ in $\poly$ for each $V$. For naturality at a $\pvect$-isomorphism $\phi\colon V\to W$, the on-positions square commutes by definition. On directions at $v$, we must check that the two composites $T^*_{\phi(v)}W\to V$ agree:
\begin{align*}
...
\begin{lemma}[Store-state interpretation]\label{lem.store_state_interpretation}
Let $\pnla$ act on $\poly$ by $P\cdot x\coloneqq\cot{P}\otimes x$. For the Para category of this action (recalled in \cref{sec.para_general}), there is a strong symmetric monoidal functor
\[
\para{\pnla}{\poly}\To{(*)}\org
\]
whose object part is the identity. It sends a morphism $(P,h)\colon x\to y$, with $h\colon\cot{P}\otimes x\to y$, to the $\ihom{x,y}$-coalgebra with state set $P$ classified by the adjoint map $\bar h\colon\cot{P}\to\ihom{x,y}$ and the isomorphism $\cot{P}\cong P\yon^P$ of \cref{prop.pnla_polynomial}.
\end{lemma}

\begin{proof}
Closedness gives $\bar h\colon\cot{P}\to\ihom{x,y}$. Applying \cref{prop.pnla_polynomial} identifies $\cot{P}\cong P\yon^P$, and \cref{ex.coalg_as_poly_map} presents $\bar h$ as an $\ihom{x,y}$-coalgebra with state set $P$. The identity case is the unit parameter $\rr^0:\pnla$ together with $\cot{\rr^0}\cong\yon$. For composition, the Para composite of $(P,h)\colon x\to y$ and $(Q,k)\colon y\to z$ is classified by
\[
\cot{Q\oplus P}\otimes x\To{\cong}\cot{Q}\otimes\cot{P}\otimes x\To{\id\otimes h}\cot{Q}\otimes y\To{k}z,
\]
which is exactly the polynomial map corresponding, under closedness, to the composite of the two coalgebras in $\org$ (\cref{prop.org}).

The same comparison proves strong monoidality; the unit comparison is again $\cot{\rr^0}\cong\yon$. Under \cref{prop.pnla_polynomial}, the strong-monoidal comparison $\cot{P\oplus Q}\cong\cot{P}\otimes\cot{Q}$ from \cref{prop.cot_monoidal} becomes $(P\yon^P)\otimes(Q\yon^Q)\cong(P\times Q)\yon^{P\times Q}$, the comparison used by the monoidal structure of $\org$ in \cref{prop.org}.\qedhere
\end{proof}

\begin{proposition}[Cotangent learners]\label{prop.cotangent_learners}
There is a strong symmetric monoidal functor
\[
\Psi\colon\para{\pnla}{\mfd}\to\org
\]
defined as the composite
\[
\para{\pnla}{\mfd}\To{\para{\pnla}{\cot}}\para{\pnla}{\poly}\To{(*)}\org,
\]
where $\para{\pnla}{\mfd}$ is the category of $\pnla$-parameterized smooth maps (the general Para construction is recalled in \cref{sec.para_general}). Its object part is $M\mapsto\cot{M}$. On a morphism $(P,f)\colon M\to N$ in $\para{\pnla}{\mfd}$, i.e.\ a paired vector space $P$ and a smooth map $f\colon P\times M\to N$, $\Psi(P,f)$ is the $\ihom{\cot{M},\cot{N}}$-coalgebra with state set $P$ classified by
\[
\cot{P}\otimes\cot{M}\To{\cong}\cot{P\times M}\To{\cot{f}}\cot{N}.
\]
At a state $p\in P$, its action is $\cot{f_p}\colon\cot{M}\to\cot{N}$, where $f_p(m)\coloneqq f(p,m)$. On a direction $(m,\nu)$ with $m\in M$ and $\nu\in T^*_{f(p,m)}N$, its update is
\[
\upd_p(m,\nu)=p+\sharp_P\bigl((\partial_Pf)_{(p,m)}^\top\nu\bigr).
\]
\end{proposition}

\begin{proof}
The action of $\pnla$ on $\mfd$ is $P\cdot M=P\times M$ by \cref{prop.pnla_smc_action}. By \cref{prop.cot_monoidal}, the cotangent functor is strong monoidal and sends this action to the action of $\pnla$ on $\poly$ by $P\cdot q=\cot{P}\otimes q$. Thus it induces the strong symmetric monoidal functor $\para{\pnla}{\cot}$ in the displayed composite. Composing with \cref{lem.store_state_interpretation} gives the claimed strong symmetric monoidal functor $\Psi$.

It remains only to unwind the displayed formula. Fixing $p$ in the position part of $\cot{f}$ gives the action $\cot{f_p}$. For a direction $\nu\in T^*_{f(p,m)}N$, the backward part of $\cot{f}$ is
\[
(Df)_{(p,m)}^\top\nu=\bigl((\partial_Pf)_{(p,m)}^\top\nu,\;(\partial_Mf)_{(p,m)}^\top\nu\bigr)\in T_p^*P\oplus T_m^*M.
\]
The adjoint map to $\ihom{\cot{M},\cot{N}}$ keeps the $P$-component, a covector $\alpha\coloneqq(\partial_Pf)_{(p,m)}^\top\nu\in T^*_pP$, as the direction in $\cot{P}$. Under the iso $\cot{P}\cong P\yon^P$ of \cref{prop.pnla_polynomial}, $\alpha$ at position $p$ is identified directly with the new state $p+\sharp_P(\alpha)$, giving the stated formula.\qedhere
\end{proof}

\subsection{Example: deep learning as a dynamic monoidal category}\label{sec.deep_learning}

We recast the deep-learning example of~\cite{shapiro2022dynamic} as the restriction of \cref{prop.cotangent_learners} to Euclidean spaces. Take the object set $O\coloneqq\nn$ and assign to each $n\in\nn$ the polynomial
\[
\Psi(\rr^n)=\cot{\rr^n}\cong{\cot{\rr}}\tpow n.
\]
A layer $m\to n$ is a morphism $\rr^m\to\rr^n$ in $\para{\pnla}{\mfd}$: a parameter space $P=\rr^k$, regarded as a paired vector space with $\sharp_P=-\eta\cdot\id$ for a learning rate $\eta>0$, together with a smooth architecture
\[
f\colon P\times\rr^m\to\rr^n.
\]

Applying $\Psi$ gives an $\ihom{\cot{\rr^m},\cot{\rr^n}}$-coalgebra with state set $P$. At $p\in P$, its action is
\[
\cot{f_p}\colon\cot{\rr^m}\to\cot{\rr^n},
\]
whose forward part is $x\mapsto f_p(x)$ and whose backward part is $(Df_p)_x^\top$.

The update at $p$ on a direction $(x,\nu)\in\rr^m\times T^*_{f_p(x)}\rr^n$ is
\[
\upd_p(x,\nu)=p+\sharp_P\bigl((\partial_Pf)_{(p,x)}^\top\nu\bigr)=p-\eta\cdot(\partial_Pf)_{(p,x)}^\top\nu,
\]
by \cref{prop.cotangent_learners}. When $\nu=dL|_{f_p(x)}$ for a loss $L\colon\rr^n\to\rr$, the chain rule gives $(\partial_Pf)_{(p,x)}^\top\nu=d_p(L\circ f(-,x))$; under the Euclidean identification, $\upd_p$ is one step of stochastic gradient descent.
...
\]
Thus a morphism $f\colon\lensob{L}\to\lensob{M}$ in $\potlens$ with parameter PN Lie Algebra $(P,\sharp_P)$ gives a polynomial map
\[
...
Applying \cref{prop.pnla_polynomial} to the paired vector space $(P,\sharp_P)$ gives
\[
\cot{P}\cong P\yon^P.
\]
...
\]
Composing with the coalgebra interpretation from \cref{prop.pnla_polynomial,ex.coalg_as_poly_map}, applied to $F(P)$, yields a functor $\Para_{\pnla,\,\cot}(\poly)\to\org$ with state set $F(P)$.
\end{proposition}

\begin{proof}
$\rho^*$ is identity on objects, sending a morphism $(P,f\colon\cot{P}\otimes x\to y)$ to $(P,\,f\circ(\rho_P\otimes\id))\colon\cot{F(P)}\otimes x\to y$.

\emph{Identity preservation.} The Para identity on $x$ has parameter $\rr^0$ (the unit of $\pnla$) and morphism the action unitor $\cot{\rr^0}\otimes x\cong x$. Strong monoidality of $F$ gives $F(\rr^0)\cong\rr^0$, and monoidality of $\rho$ identifies $\rho_{\rr^0}$ with the canonical $\cot{F(\rr^0)}\cong\cot{\rr^0}$. Hence $\rho^*(\id_x)$ equals the action unitor for $\cot\circ F$, i.e., the identity in $\Para_{\pnla,\cot\circ F}(\poly)$.

\emph{Composition preservation.} For composable Para morphisms $(P,f)\colon x\to y$ and $(Q,g)\colon y\to z$, the composite $(Q\oplus P,h)$ has
\[
h\colon\cot{Q\oplus P}\otimes x\To{\sigma\otimes\id}\cot{Q}\otimes\cot{P}\otimes x\To{\id\otimes f}\cot{Q}\otimes y\To{g}z,
\]
where $\sigma\colon\cot{Q\oplus P}\To{\cong}\cot{Q}\otimes\cot{P}$ is the strong-monoidal comparison from \cref{prop.cot_monoidal}. The Para composite of $\rho^*(P,f)$ and $\rho^*(Q,g)$ in $\Para_{\pnla,\cot\circ F}(\poly)$ uses the analogous comparison $\sigma_F\colon\cot{F(Q\oplus P)}\To{\cong}\cot{F(Q)}\otimes\cot{F(P)}$ for the action $P\mapsto\cot{F(P)}$ and reads
\[
\cot{F(Q\oplus P)}\otimes x\To{\sigma_F\otimes\id}\cot{F(Q)}\otimes\cot{F(P)}\otimes x\To{(\rho_Q\otimes\rho_P)\otimes\id}\cot{Q}\otimes\cot{P}\otimes x\To{\id\otimes f}\cot{Q}\otimes y\To{g}z.
\]
Monoidality of $\rho$ asserts that the square
\[
\begin{tikzcd}[ampersand replacement=\&]
\cot{F(Q\oplus P)}\ar[r,"\rho_{Q\oplus P}"]\ar[d,"\sigma_F"']\&\cot{Q\oplus P}\ar[d,"\sigma"]\\
\cot{F(Q)}\otimes\cot{F(P)}\ar[r,"\rho_Q\otimes\rho_P"']\&\cot{Q}\otimes\cot{P}
\end{tikzcd}
\]
commutes; this identifies $\rho^*(Q\oplus P,h) = (Q\oplus P,h\circ(\rho_{Q\oplus P}\otimes\id))$ with the Para composite of $\rho^*(P,f)$ and $\rho^*(Q,g)$.

\emph{Coalgebra interpretation.} For a morphism $(P,h\colon\cot{F(P)}\otimes x\to y)$ in $\Para_{\pnla,\cot\circ F}(\poly)$, applying \cref{prop.pnla_polynomial} to the paired vector space $F(P)\in\pnla$ gives $\cot{F(P)}\cong F(P)\yon^{F(P)}$, so $h$ is a polynomial map $F(P)\yon^{F(P)}\otimes x\to y$. By \cref{ex.coalg_as_poly_map} this presents $h$ as an $\ihom{x,y}$-coalgebra with state set $F(P)$, equivalently a 1-cell in $\org$. Functoriality is direct: composition of two such morphisms uses $\cot{F(Q\oplus P)}\cong\cot{F(Q)}\otimes\cot{F(P)}$ together with \cref{prop.pnla_polynomial}, which corresponds to coalgebra composition in $\org$.\qedhere
\end{proof}
...
T^*P\cong P\oplus P^*.
\]
We call a point $(u,\pi)$ a pair consisting of a \emph{position} $u\in P$ and a \emph{momentum} $\pi\in P^*$. The cotangent space there decomposes as
\[
T^*_{(u,\pi)}(T^*P)\cong T^*_uP\oplus T^*_\pi(P^*)\cong P^*\oplus P,
\]
both identifications by \eqref{eqn.tangent_vec}, using $(P^*)^*\cong P$ on the $P^*$-factor.
...
\[
T^*_uP\cong P^*\To{\sigma\mapsto(\sigma,\,\sharp_P(\pi))}P^*\oplus P\cong T^*_{(u,\pi)}(T^*P).
\]
\end{definition}

On positions, $\rho_P$ forgets momentum; on directions, it passes the incoming covector $\sigma$ through to the $P^*$-slot and injects the pairing-lifted momentum $\sharp_P(\pi)\in P$ into the $P$-slot, independently of $\sigma$. Thus $\rho_P$ is not of the form $\cot{h}$ for any smooth $h\colon T^*P\to P$: the $P$-component of its direction-action is constant in $\sigma$, whereas $\cot$ always produces a linear direction-action.
...
\ihom{\Phi_1\lensob{L},\Phi_1\lensob{M}}.
\]
Since $T^*P$ is a paired vector space by \cref{prop.canonical_symplectic_pairing}, \cref{prop.pnla_polynomial} gives
\[
...
Taking $F\coloneqq T$ (\cref{prop.TT_endofunctors}) instead of $T^*$ fits \cref{prop.param_refinement} equally well. The analogous \emph{tangent projection} $\tilde\rho_P\colon\cot{TP}\to\cot{P}$ has position-action $(u,X)\mapsto u$ and direction-action $\sigma\mapsto(\sigma,\,\flat_P(X))$ under the decomposition $T^*_{(u,X)}(TP)\cong P^*\oplus P^*$ (both factors via \eqref{eqn.tangent_vec}), using the flat map $\flat_P\colon P\to P^*$ to place $X$ in the second summand. This produces a functor $\potlens\to\org$ with state set $TP$, giving a Lagrangian-style (second-order) state update instead of Hamilton's equations. Beyond existence, we do not develop this alternative in the paper. The category-theoretic framework of \cref{prop.param_refinement} does not, by itself, single out $T^*$ over $T$; the preference for $T^*$ here is physics (cotangent bundles carry the canonical symplectic pairing of \cref{prop.canonical_symplectic_pairing}, and the resulting dynamics coincide with textbook Hamilton's equations).
\end{remark}
...
\begin{proof}
By construction in \cref{ch.potlens_to_org}, $\Phi$ is the composite
\[
\potlens=\para{\pnla}{\lmfd^\rr}
\To{\para{\pnla}{\Phi_1}}
\para{\pnla}{\poly}
\To{\rho^*}
\para{\pnla}{\poly}
\To{(*)}\org,
\]
where $\Phi_1\colon\lmfd^\rr\to\poly$ is the composite of the cotangent lift $\widehat\cot\colon\lmfd^\rr\to\Lens\poly^{\Fun T}$ (induced from \cref{prop.cot_monoidal} by lensification, with the writer monad on $\rr$ transported via $\cot{0},\cot{+}$ from \cref{prop.cot_hopf}) with $\Theta^{\Fun T,\dec}\colon\Lens\poly^{\Fun T}\to\poly$ (\cref{prop.Theta_T_alpha}); $\rho^*$ is the parameter refinement of \cref{prop.param_refinement} for $F\coloneqq T^*$ and $\rho$ the pairing projection of \cref{def.pairing_proj}; and $(*)$ is the coalgebra interpretation $(P,f)\mapsto$ ($\ihom{x,y}$-coalgebra with state $T^*P$) from \cref{prop.pnla_polynomial} and \cref{ex.coalg_as_poly_map}, applied at $T^*P$.

Each step is a functor: $\Phi_1$ as a composite of functors; $\para{\pnla}{\Phi_1}$ as the standard Para lift of a functor that preserves the actions; $\rho^*$ by the first half of \cref{prop.param_refinement}; and $(*)$ by the second half. Their composite is $\Phi$.\qedhere
\end{proof}
...
\begin{proof}
$\potlens=\para{\pnla}{\lmfd^\rr}$. The Para construction over a symmetric monoidal action of an SMC on an SMC is itself symmetric monoidal: unit equals the unit of the underlying category (acted on by the unit of the action), tensor is parameterwise and slotwise on objects, and tensor of morphisms uses the strong-monoidal comparison of the action $(a\otimes b)\cdot(x\otimes z)\cong(a\cdot x)\otimes(b\cdot z)$ to interleave parameters; coherence is inherited componentwise from $\cat A$, $\cat D$, and the action.

Here $\pnla$ is SMC by \cref{prop.pnla_monoidal}, and $\lmfd^\rr=\Lens\mfd^\rr$ inherits an SMC structure from $(\mfd,\times)$ via lensification together with the writer monad on $\rr$, whose monoid structure $0\colon\rr^0\to\rr$ and $+\colon\rr\times\rr\to\rr$ make $\rr$ a symmetric monoid object in $\mfd$. The $\pnla$-action on $\lmfd^\rr$ factors through the strong monoidal forgetful $\pnla\to\mfd$ (\cref{prop.pnla_smc_action}) and is symmetric monoidal. Reading off the inherited structure: the unit is $\binom{\rr^0}{\rr^0}$, and the tensor of $(P,\lensob L)$ with $(P',\lensob{L'})$ is $(P\oplus P',\lensobs{L}{L'})$, with independent forward $\outp f\times\outp{f'}$, independent backward $\inpt f\times\inpt{f'}$, and additive potential $V^\otimes=V\circ\pr+V'\circ\pr'$ from the writer's monoid structure.\qedhere
\end{proof}
...
\begin{proof}
We use the chain of functors from \cref{thm.functor}'s proof. Each piece is monoidal of the indicated strength:
\begin{itemize}
\item $\Phi_1\colon\lmfd^\rr\to\poly$ is lax symmetric monoidal: $\widehat\cot$ is strong (induced from \cref{prop.cot_monoidal} and the writer-monad transport via \cref{prop.cot_hopf}), and $\Theta^{\Fun T,\dec}$ is lax (\cref{prop.Theta_T_alpha}); composition of strong with lax is lax.
\item $\para{\pnla}{\Phi_1}$ is lax symmetric monoidal: the Para lift of a (lax/strong) symmetric monoidal action-preserving functor is (lax/strong) of the same strength, since the laxator and unit comparison transport componentwise through the strong-monoidal action interchange isomorphism.
\item $\rho^*$ is symmetric monoidal: the proof of \cref{prop.param_refinement} establishes that the Para tensor is preserved by $\rho^*$, with the necessary commutative square supplied by monoidality of $\rho$. Here the pairing projection $\rho$ is monoidal because $\sharp_{P_1\oplus P_2}=\sharp_{P_1}\oplus\sharp_{P_2}$ (\cref{prop.TT_monoidal}) and $T^*(P_1\oplus P_2)\cong T^*P_1\oplus T^*P_2$ (\cref{prop.TT_monoidal} again).
\item The coalgebra interpretation $(*)$ is strong symmetric monoidal: \cref{prop.pnla_polynomial} gives $\cot{P}\cong P\yon^P$, and the strong-monoidal comparison $\cot{P\oplus Q}\cong\cot{P}\otimes\cot{Q}$ from \cref{prop.cot_monoidal} corresponds to the polynomial-tensor comparison $(P\yon^P)\otimes(Q\yon^Q)\cong(P\times Q)\yon^{P\times Q}$, which is the comparison used to define the SMC structure on $\org$ (\cref{prop.org}).
\end{itemize}
Composing strong $\circ$ monoidal $\circ$ lax $\circ$ strong yields a lax symmetric monoidal composite. Only $\Theta^{\Fun T,\dec}$ contributes nontrivial laxness; its forward laxator is the closedness pairing $\ihom{m,I}\otimes\ihom{n,I}\to\ihom{m\otimes n,I}$ of \cref{prop.Theta} (combined with the writer's $\tau_{I,I}$ and algebra $\dec$). Pulled through the strong identifications, this gives the morphism laxator stated in the theorem: state component $T^*P\otimes T^*P'\cong T^*(P\oplus P')$ from \cref{prop.TT_monoidal}; cotangent-bundle component $\cot{\outp L}\otimes\cot{\outp{L'}}\cong\cot{\outp L\times\outp{L'}}$ from \cref{prop.cot_monoidal}; hom-factor component $\ihom{\cot{\inpt L},\yon}\otimes\ihom{\cot{\inpt{L'}},\yon}\to\ihom{\cot{\inpt L\times\inpt{L'}},\yon}$ from \cref{prop.Theta}. Coherence (associator, unitor, symmetry) is inherited from $(\mfd,\times)$ and $(\poly,\otimes)$ via the strong comparisons. The hom pairing is not invertible (\cref{prop.Theta} is lax, not strong), so $\Phi$ is lax, not strong.\qedhere
\end{proof}
...
The pairing $\sharp_P(p)=p/\mu$ is chosen, not the standard Euclidean pairing $\sharp_P(p)=p$. The free parameter $\mu$ encodes inverse mass; the standard pairing corresponds to $\mu=1$.
\end{remark}
...
\begin{proof}
Unwinding $\Phi(f_\mathrm{spring})$ at $(x,p)$: the $\sharp_P(p)=p/\mu$ component of $\dot x$ is the $P$-slot of the pairing projection (\cref{def.pairing_proj}). The $\dot p$ component receives contributions from $V$, $\outp f$, and $\inpt f$ along the chain induced by $\rho_P$; only the $V$-contribution is nonzero here ($\partial_P\inpt f=0$ and $\nu=0$), giving $\dot p=-\partial_xV=\kappa(\ell+r-2x)$.\qedhere
\end{proof}
