...
\begin{proof}
Use the supplied commutative comonoid on $J(v)\in\cat C$; by homomorphicity, $J(u)$ is a comonoid homomorphism for any $u\colon v\to w$ in $\cat V$. Given $f\colon\lensob c\coto\lensob d$ in $\Lens{\cat C}^{\Fun T}$ and $u$ as above, set
\[
...
show composition on the following page. We need to prove that two morphisms $\outp c\otimes H_c\to\outp e\otimes H_e$ (writing $H_X\coloneqq[\inpt X,z]$) are equal: $\Theta_z$ applied to the coKleisli composite $g\circ f$ and the sequential $\Theta_z(g)\circ\Theta_z(f)$. 

Each ``tile'' is of one of three types: functoriality of $\otimes$; naturality of $\eta$, $\mu$, and $\tau$; and the $\Fun T$-monoid axioms of \cref{def.T_monoid}. 
...
\end{equation}
\end{corollary}
%It sends a parameterized coKleisli lens $(v,f\colon v\cdot\lensob c\coto\lensob d)$ to the parameterized $\cat C$-morphism
%\[
...
\begin{proof}
\cref{prop.cot_monoidal,lem.cot_comonoid} give $\cot\colon\pvect\to\poly$; \cref{lem.Theta_poly_potential} gives the lax monoidal $\Theta_\dec$; and \cref{cor.parameterized_representation} gives the induced functor.
\end{proof}
...
\iffalse
The $\pvect$-action on $\lmfd^\rr$ defined above and the action $V\cdot p\coloneqq\cot V\otimes p$ on $\poly$ fit into the parameterized representation of \cref{cor.parameterized_representation}: the comparison
\[
