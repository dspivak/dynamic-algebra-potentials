...
\[
\cint\colon\para{\pvect}{\lmfd^\rr}\to\para{\cot}{\poly}.
\]
\end{lemma}
...
The action of $\Phi$ on objects is:
\[
\Phi\lensob M=
...
be a morphism in $\para{\pvect}{\lmfd^\rr}$, i.e.\ a tuple $(\outp f,\inpt f,U)$ as in \eqref{eqn.para_potential_lens_maps}:
...
\]
we write $x=(v,\xi)\in X$ for position $v\in V$ and momentum $\xi\in V^*$, and equip $X$ with the canonical symplectic pairing (\cref{prop.canonical_symplectic_pairing}), whose sharp map $\sharp_X\colon X^*\to X$, $(\xi',v')\mapsto(v',-\xi')$, is \eqref{eqn.canonical_sharp}.%
\footnote{This comes from \cref{def.legendre_projection}.}
...
We illustrate $\Phi$ with a one-dimensional spring cell: a mass coupled to one neighbor by a Hooke spring. The state space comes out to $T^*\rr$, and the dynamics are Hamilton's equations for the harmonic oscillator.
