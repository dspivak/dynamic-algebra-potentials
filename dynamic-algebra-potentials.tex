...
\]
which at $v\in V$ sends a point $m\in M$ to  $n\coloneqq f(v,m)\in N$ and for a covector $\beta\in T^*_nN$ if we let $(\xi,\alpha)\coloneqq(Df)_{(v,m)}^\top(\beta)\in T_v^*V\times T_m^*M$, returns $\alpha\in T^*_mM$ and updates to
\begin{equation}\label{eqn.cotangent_learner_update}
\upd_v(m,\beta)\coloneqq v+\sharp_V(\xi).
\end{equation}
\end{corollary}
...
\begin{definition}\label{def.lens_cokleisli}
For any strong monad $\Fun T$ on a symmetric monoidal category $\cat{C}$, we denote the coKleisli category of $\Lens{\Fun{T}}$ by $\Lens{\cat{C}}^{\Fun{T}}\coloneqq\left(\Lens{\cat{C}}\right)_{\Lens{\Fun T}}$. 
\end{definition}
...
Let $\cat M$ be symmetric monoidal, let $\cat C$ be symmetric monoidal with a homomorphic supply of commutative comonoids, let $\Fun T$ be a strong monad on $\cat C$, and let $J\colon\cat M\to\cat C$ be strong monoidal. Then we have an action of $\cat M$ on $\Lens{\cat C}^{\Fun T}$ given by
\[
m\cdot\lensob c\coloneqq\binom{\inpt c}{J(m)\otimes\outp c}
\]
\end{lemma}

\begin{proof}
Use the supplied commutative comonoid on $J(m)\in\cat C$; by homomorphicity, $J(u)$ is a comonoid homomorphism for any $u\colon m\to n$ in $\cat M$. Given $f\colon\lensob c\coto\lensob d$ in $\Lens{\cat C}^{\Fun T}$ and $u$ as above, set
\[
u\cdot f\coloneqq\binom{\inpt f\circ\epsilon_{J(m)}}{J(u)\otimes\outp f}\colon
\binom{\inpt c}{J(m)\otimes\outp c}
\coto
\binom{\inpt d}{J(n)\otimes\outp d}.
\]
Checking functoriality reduces immediately to the counital projection \eqref{eqn.counital_projection} and the homomorphic supply assumption: $J(m)\To{J(u)}J(n)\To{\epsilon_{J(n)}} I$ equals $\epsilon_{J(m)}$.
\end{proof}
...
\end{proposition}
\begin{proof}
We need to prove the equality $\Theta(g\circ f)=\Theta(g)\circ\Theta(f)\colon\outp c\,H_c\to\outp e\,H_e$ for $f\colon\lensob c\coto\lensob d$ and $g\colon\lensob d\coto\lensob e$. It amounts to the commutativity of the outer square below, where we write $H_x\coloneqq[\inpt x,I]$, and for space reasons write $c,d,e$ for $\inpt c,\inpt d,\inpt e$ and elide the $\otimes$-symbol, so e.g.\ $\outp d\outp d[d,c H_c]$ means $\outp d\otimes\outp d\otimes[\inpt d,\inpt c\otimes [c,I]]$. 
\[
...
\& \outp d[d,d\outp c H_c] \ar[r, "{\boldsymbol{f}}"] \ar[d, "{\boldsymbol\delta}"']
\& \outp d[d,c H_c] \ar[r, "{\boldsymbol{\ev}}"] \ar[d, "{\boldsymbol\delta}"']
\& \outp d H_d \ar[d, "{\boldsymbol\delta}"] \\
\outp c\outp c H_c \ar[d, "{\boldsymbol{\outp f}}"'] \ar[r, "{\boldsymbol\delta}"]
...
\& \outp d\outp d[d,d\outp c H_c] \ar[r, "{\boldsymbol{f}}"] \ar[d, "{\boldsymbol{\outp g}}"']
\& \outp d\outp d[d,c H_c] \ar[r, "{\boldsymbol{\ev}}"] \ar[d, "{\boldsymbol{\outp g}}"']
\& \outp d\outp d H_d \ar[d, "{\boldsymbol{\outp g}}"] \\
\outp d\outp c H_c \ar[d, "{\boldsymbol{\outp g}}"'] \ar[r, "{\boldsymbol\delta}"]
...
\& \outp e\outp d[d,d\outp c H_c] \ar[r, "{\boldsymbol{f}}"] \ar[d, "{\boldsymbol{\coev_e}}"']
\& \outp e\outp d[d,c H_c] \ar[r, "{\boldsymbol{\ev}}"] \ar[d, "{\boldsymbol{\coev_e}}"']
\& \outp e\outp d H_d \ar[d, "{\boldsymbol{\coev_e}}"] \\
\outp e\outp c H_c \ar[d, "{\boldsymbol{\coev_e}}"']
\& \outp d[e,e\outp cH_c]\ar[from=ul, "{\boldsymbol{\coev_e}}"]\ar[dl, "\boldsymbol{\outp g}"]\ar[d,"\boldsymbol{\delta}"]
\& \outp e[e,e\outp d\outp c H_c] \ar[r, "{\boldsymbol{\coev_d}}"] \ar[d, "{\boldsymbol{g}}"'] \ar[dd, equal, bend right=60pt]
\& \outp e[e,e\outp d[d,d\outp c H_c]] \ar[r, "{\boldsymbol{f}}"] \ar[d, "{\boldsymbol{g}}"']
\& \outp e[e,e\outp d[d,c H_c]] \ar[r, "{\boldsymbol{\ev}}"] \ar[d, "{\boldsymbol{g}}"']
\& \outp e[e,e\outp d H_d] \ar[d, "{\boldsymbol{g}}"] \\
\outp e[e,e\outp c H_c] \ar[d, "{\boldsymbol\delta}"']
\& \outp d[e,e\outp c\outp cH_c]\ar[ld, "{\boldsymbol{\outp g}}"]\ar[ur, "\boldsymbol{\outp g\outp f}"', pos=.3]\ar[from=uu, , bend left=60pt, "{\boldsymbol{\coev_e}}", pos=.3]
\& \outp e[e,d\outp c H_c] \ar[r, "{\boldsymbol{\coev_d}}"] \ar[dr, equal]
\& \outp e[e,d[d,d\outp c H_c]] \ar[r, "{\boldsymbol{f}}"] \ar[d, "{\boldsymbol{\ev}}"']
\& \outp e[e,d[d,c H_c]] \ar[r, "{\boldsymbol{\ev}}"] \ar[d, "{\boldsymbol{\ev}}"']
\& \outp e[e,d H_d] \ar[d, "{\boldsymbol{\ev}}"] \\
\outp e[e,e\outp c\outp c H_c] \ar[rr, "{\boldsymbol{\outp f}}"]
\&\& \outp e[e,e\outp d\outp c H_c] \ar[r, "{\boldsymbol{g}}"]
\& \outp e[e,d\outp c H_c] \ar[r, "{\boldsymbol{f}}"]
\& \outp e[e,c H_c] \ar[r, "{\boldsymbol{\ev}}"]
\& \outp e H_e
\end{tikzcd}
\]
\end{proof}
...
Let $(\Fun{T},\tau)$ be a monoidal monad on a symmetric monoidal closed category $(\cat{C},I,\otimes, [-,-])$, and let $(z,e_z,m_z,\alpha)$ be a $\Fun{T}$-monoid in $\cat{C}$ (\cref{def.T_monoid}). Then there is a lax monoidal functor that acts on objects as follows:
\[
\Theta\colon\Lens{\cat{C}}^\Fun{T}\to\cat{C},\qquad
\lensob{c}\mapsto\outp c\otimes\ihom{\inpt c,z}.
\]
\end{proposition}
\begin{proof}
Given a coKleisli morphism $f\colon\lensob{c}\coto\lensob{d}$, define $\Theta(f)$ as the composite
\[
\begin{tikzcd}[column sep=45pt, row sep=22pt, ampersand replacement=\&]
\outp c\otimes[\inpt c,z]\ar[r,"{\delta_{\outp c}\otimes\id}"]\ar[dd, dashed, "\Theta(f)"']
...
\end{tikzcd}
\]
For identities, write $H\coloneqq[\inpt c,z]$. The diagram to prove is
...
|[alias=tl]| \outp c\otimes H
  \ar[rr, "{\boldsymbol{\delta}}"]
  \ar[ddd, equal]
  \ar[drr, equal]
\&
\& \outp c\otimes\outp c\otimes H
  \ar[r, "{\mathbf{coev}}", ""{description, name=A1, anchor=center, inner sep=0}]
  \ar[d, "{\boldsymbol{\varepsilon}}" description]
\& \outp c\otimes[\inpt c,\inpt c\otimes\outp c\otimes H]
  \ar[d, "{\boldsymbol{\varepsilon}}"]
\\
\&
\& |[alias=ce]| \outp c\otimes H
  \ar[r, "{\mathbf{coev}}", ""{description, name=A2, anchor=center, inner sep=0}]
  \ar[d, "{\boldsymbol{\eta}_H}"']
  \ar[dl, "{\mathbf{coev}}"' description]
  \ar[ddll, equal, out=180, in=60]
\& \outp c\otimes[\inpt c,\inpt c\otimes H]
  \ar[d, "{\boldsymbol{\eta}_{\inpt c}}"]
  \ar[ddl, "{\boldsymbol{\eta}_H}"' description]
\\
\& \outp c\otimes[\inpt c,\inpt c\otimes H]
  \ar[dl, "{\boldsymbol{\ev}}"]
  \ar[dr, "{\boldsymbol{\eta}}"]
  \ar[ddr, "{\boldsymbol{\eta}_{\inpt c}}"' description]
  \ar[dddr, bend right=20, "{\boldsymbol{\eta}_{\inpt c\otimes H}}"' description]
\& \outp c\otimes\Fun{T}H
  \ar[d, "{\mathbf{coev}}"]
\& \outp c\otimes[\inpt c,\Fun{T}\inpt c\otimes H]
  \ar[d, "{\boldsymbol{\eta}_H}", ""{description, name=DR, anchor=center, inner sep=0}]
\\
\outp c\otimes[\inpt c,z]
  \ar[ddr, "{\boldsymbol{\eta}_z}"]\ar[ddd, equal]
\&\& \outp c\otimes[\inpt c,\inpt c\otimes\Fun{T}H]
  \ar[r, "{\boldsymbol{\sigma^{L}}}", ""{description, name=DL, anchor=center, inner sep=0}]
\& \outp c\otimes[\inpt c,\Fun{T}\inpt c\otimes\Fun{T}H]
  \ar[ddl, "{\boldsymbol{\tau}}"]
\\
\&
\& \outp c\otimes[\inpt c,\Fun{T}\inpt c\otimes H]
  \ar[ur, "{\boldsymbol{\eta}_H}"]
\&
\\
\& |[alias=mz]| \outp c\otimes[\inpt c,\Fun{T}z]
  \ar[dl, "{\boldsymbol{\alpha}}"' description]
\& \outp c\otimes[\inpt c,\Fun{T}(\inpt c\otimes H)]
  \ar[l, "{\boldsymbol{\Fun{T}\ev}}"' description]
\&
\\
\outp c\otimes H
\&
...
For composition, the obligation is the equality of two morphisms $\outp c\,H_c\to\outp e\,H_e$ (writing $H_X\coloneqq[\inpt X,z]$): $\Theta$ applied to the coKleisli composite $g\circ f$ vs the sequential $\Theta(g)\circ\Theta(f)$. The two atomized boundaries (each arrow performs exactly one nonidentity operation, in bold; juxtaposition denotes $\otimes$; $\sigma$ has been opened as $\eta$ then $\tau$) are:

The top--right boundary unfolds $\Theta(g)\circ\Theta(f)$: the f-half (atoms $\delta,\coev_d,\outp f,\eta,\inpt f,\tau,\Fun T\ev,\delta$) produces $H_d$, the bridge $(\eta, \Fun T\alpha)$ applies $\eta$-naturality at $\alpha$ to lift the intermediate $\alpha$ inside $\Fun T$, and the g-half ($\outp g,\coev_e,\inpt g,\tau,\Fun T\ev$) produces $H_e$. The left--bottom boundary unfolds $\Theta(g\circ f)$ directly via the Kleisli formula for $\inpt{(g\circ f)}$ involving $\Fun T\inpt f,\mu$.

The interior 2-cells fall into three classes: (i)~bifunctoriality of $\otimes$ on disjoint slots, which commutes any two atoms acting on different tensor factors of the source; (ii)~naturality of $\eta$, $\mu$, and $\tau$ at the various structural morphisms; and (iii)~the $\Fun T$-monoid axioms of \cref{def.T_monoid}---specifically the $\Fun T$-algebra associativity $\alpha\circ\Fun T\alpha=\alpha\circ\mu_z$ bridges the path through $H_d$ (TR) with the direct $\mu_z$-path (LB).
\begin{landscape}
\[
...
The lax unit comparison is $e_z\colon I\to z=\Theta\binom{I}{I}$, and the lax laxator is the canonical map
\[
...
\]
combining closedness with the monoid multiplication on $z$. Compatibility with morphisms is exactly monoidality of $\Fun{T}$ together with the assumption that $\alpha$ is a monoid homomorphism; the unit law uses $\alpha\circ\Fun{T}e_z\circ\tau_0=e_z$.
\end{proof}

\begin{proposition}[Parameterized representation]\label{prop.parameterized_representation}
Let $(\Fun T,\tau)$, $\cat C$, and $(z,e_z,m_z,\alpha)$ be as in \cref{prop.Theta_T_alpha}, with $\cat C$ carrying a homomorphic supply of commutative comonoids. Suppose $\cat M$ is symmetric monoidal and $J\colon\cat M\to\cat C$ is strong monoidal. Then with the $\cat M$-actions
\[
m\cdot\lensob c\coloneqq\binom{\inpt c}{J(m)\otimes\outp c}
\qqand
m\cdot x\coloneqq J(m)\otimes x
\]
on $\Lens{\cat C}^{\Fun T}$ and on $\cat C$ respectively, $\Theta\colon\Lens{\cat{C}}^\Fun{T}\to\cat{C}$  extends to a lax symmetric monoidal functor
\[
\para{\cat M}{\Theta}\colon\para{\cat M}{\Lens{\cat C}^{\Fun T}}\to\para{\cat M}{\cat C}.
\]
\end{proposition}
%It sends a parameterized coKleisli lens $(m,f\colon m\cdot\lensob c\coto\lensob d)$ to the parameterized $\cat C$-morphism
%\[
%J(m)\otimes\outp{c}\otimes[\inpt{c},z]\to\outp{d}\otimes[\inpt{d},z].
%\]

\begin{proof}
Use the action on $\Lens{\cat C}^{\Fun T}$ from \cref{lem.parameter_lens_action} and the action $m\cdot x\coloneqq J(m)\otimes x$ on $\cat C$. The functor $\Theta$ is $\cat M$-equivariant:
\[
\begin{tikzcd}[ampersand replacement=\&]
\cat M\times\Lens{\cat C}^{\Fun T}
  \ar[r,"(\cdot)"]
  \ar[d,"\id_{\cat M}\times\Theta"']
  \ar[dr,phantom,"\theta"description]
\& \Lens{\cat C}^{\Fun T}\ar[d,"\Theta"]\\
\cat M\times\cat C
  \ar[r,"(\cdot)"']
\& \cat C,
\end{tikzcd}
...
where
\[
\theta_{m,\lensob c}\colon
J(m)\otimes\Theta\lensob c
\cong J(m)\otimes\outp c\otimes\ihom{\inpt c,z}
=\Theta(m\cdot\lensob c)
\]
is compatible with action unitors and associators by coherence for the strong monoidal structure of $J$. Thus \cref{prop.para_square}, with $F=\id_{\cat M}$, gives the displayed functor. Its lax monoidal structure is inherited from \cref{prop.Theta_T_alpha}.
\end{proof}

\section{}

\begin{lemma}\label{lemma.monoid_to_monad}
If $\cat{C}$ is a monoidal category and $(M,e,(*))$ is a monoid object in it, then $M\otimes-\colon\cat{C}\to\cat{C}$ carries a monad structure.
\end{lemma}

Recall from \cref{prop.cot_hopf} that since $(\rr,0,(+)$) is a Lie group, $\cot{\rr}$ is a monoid object in $\poly$. By \cref{lemma.monoid_to_monad} there are corresponding monads $\rr\times-$ and $\cot{\rr}\otimes-$ on $\mfd$ and $\poly$, and by \cref{def.lens_cokleisli} there are corresponding comononads on $\Lens{\mfd}$ and $\Lens{\poly}$. We let $\Lens{\mfd}^\rr$ and $\Lens{\poly}^\rr$ denote the corresponding cokleisli categories. 

Taking $\pvect\to\mfd$ from \cref{prop.pnla_smc_action} and the homomorphic supply on $\mfd$ from \cref{lem.cartesian_homomorphic_supply}, we have an action
\[(\cdot)\colon\pvect\times\mfd\to\mfd\] 
by \cref{lem.parameter_lens_action}. On objects it is given by $V\cdot\lensob M\coloneqq\binom{\inpt M}{V\times\outp M}$. Recall that a map in $\lmfd^\rr$ is of the form $f\coloneqq (\outp f,\inpt f,U)\colon\lensob M\to\lensob N$, where
\[
\outp f\colon\outp M\to\outp N,\qquad
\inpt f\colon\outp M\times\inpt N\to\inpt M,\qquad
U\colon\outp M\times\inpt N\to\rr.
\]
For $\phi\colon V\To{\cong} W$ in $\pvect$, the map $\phi\cdot f\colon V\cdot\lensob M\to W\cdot\lensob N$ is given by
\[
\outp{(\phi\cdot f)}(v,x)\coloneqq(\phi(v),\outp f(x)),\qquad
\inpt{(\phi\cdot f)}(v,x,y)=\inpt f(x,y),\qquad
U_{\phi\cdot f}(v,x,y)=U(x,y).
\]

\begin{lemma}\label{lem.potlens_to_para_poly}
There is a lax symmetric monoidal functor
\[
\para{\pvect}{\Lens{\mfd}^{\rr}}\to\para{\pvect}{\poly},
\]
where $\pvect$ acts on $\poly$ by $V\cdot p\coloneqq\cot{V}\otimes p$.
\end{lemma}

\begin{proof}
\cref{prop.cot_monoidal,lem.cot_comonoid} give $\cot\colon\pvect\to\poly$; \cref{lem.Theta_poly_potential} gives the lax monoidal $\Theta$; and \cref{prop.parameterized_representation} gives the induced functor.
\end{proof}

\begin{lemma}\label{lem.Theta_poly_potential}
There is a lax monoidal functor
\[
\Theta\colon\Lens{\poly}^\rr\to\poly,\qquad
\Theta\lensob p=\outp p\otimes\ihom{\inpt p,\yon}.
\]
\end{lemma}

\begin{proof}
\Cref{prop.cot_hopf,prop.Theta_T_alpha}.
\end{proof}

\begin{lemma}\label{lem.para_potential_lens_maps}
A morphism $\lensob L\to\lensob M$ in $\para{\pvect}{\Lens{\mfd}^{\Fun T_\rr}}$ is a tuple $(V,\outp f,\inpt f,U)$ with
\[
...
\begin{proof}
\Cref{eqn.lens_def,subsec.backwards,prop.pnla_smc_action}.
\end{proof}

\begin{theorem}\label{thm.functor}
$\Phi\colon\para{\pvect}{\Lens{\mfd}^{\Fun T_\rr}}\to\org$ is a lax symmetric monoidal functor.
\end{theorem}

\begin{proof}
\Cref{lem.potlens_to_para_poly,def.pairing_proj,prop.TT_monoidal,prop.para_square,prop.org_as_para,prop.coalg_as_poly_map,prop.pnla_polynomial,prop.cot_monoidal}.
\end{proof}

On objects,
\[
\Phi\lensob L=\cot{\outp L}\otimes\ihom{\cot{\inpt L},\yon}.
\]
\Cref{lem.Theta_poly_potential,prop.cot_monoidal}.
On morphisms,
\[
\left(V,\;f\colon V\cdot\lensob L\to\lensob M\right)
\longmapsto
X\To{\Phi(f)}
\bigl[
\cot{\outp L}\otimes\ihom{\cot{\inpt L},\yon},
...
\]
\Cref{lem.para_potential_lens_maps,prop.TT_endofunctors,prop.canonical_symplectic_pairing,prop.org_as_para,prop.coalg_as_poly_map,prop.pnla_polynomial}.
The coalgebra is induced by
\[
\tilde f\colon
\cot{T^*V}\otimes\Phi\lensob L
\To{\rho_V\otimes\id}
\cot{V}\otimes\Phi\lensob L
\To{\bar f}
\Phi\lensob M,
\]
where $\bar f\colon\cot{V}\otimes\Phi\lensob L\to\Phi\lensob M$ is the image of $f$ under \cref{lem.potlens_to_para_poly}.
\Cref{lem.potlens_to_para_poly,def.pairing_proj}.
At $x=(u,\pi)\in X$, the map $\rho_V\colon\cot{T^*V}\to\cot{V}$ sends the position $(u,\pi)\in T^*V$ to $u\in V$ and sends a covector $\sigma\in T_u^*V$ to
\[
\bk{\rho_V}_{(u,\pi)}(\sigma)=(\sigma,\sharp_V(\pi))\in T_x^*X.
\]
\Cref{def.pairing_proj}.
At $x\in X$, for a position $a\in(\Phi\lensob L)(1)$, let $b\coloneqq(\tilde f)_1(x,a)$. For a direction $\beta\in\Phi\lensob M[b]$, if
\[
(\xi,\alpha)\coloneqq\bk{\tilde f}_{(x,a)}(\beta)\in T_x^*X\times\Phi\lensob L[a],
\]
then at $(x,a)$ the coalgebra $\Phi(f)$ returns the direction $\alpha\in\Phi\lensob L[a]$ and updates $x$ to $x+\sharp_X(\xi)$.
\Cref{prop.coalg_as_poly_map,prop.pnla_polynomial}.

\begin{lemma}\label{lem.Phi_coordinate_update}
Let $f=(V,\outp f,\inpt f,U)$ be as in \cref{lem.para_potential_lens_maps}, and let $X=T^*V$. At $x=(u,\pi)\in X$, if
\[
(\sigma,\alpha)\coloneqq\bk{\bar f}_{(u,a)}(\beta)\in T_u^*V\times\Phi\lensob L[a],
\]
then
\[
\bk{\tilde f}_{(x,a)}(\beta)=\bigl((\sigma,\sharp_V(\pi)),\,\alpha\bigr)
\qqand
x+\sharp_X(\sigma,\sharp_V(\pi))=(u+\sharp_V(\pi),\,\pi-\sigma).
\]
\end{lemma}

\begin{proof}
\Cref{lem.para_potential_lens_maps,lem.potlens_to_para_poly,def.pairing_proj,prop.canonical_symplectic_pairing,prop.pnla_polynomial}.
\end{proof}
...
\footnote{As mentioned in \cref{***}, everything about paired vector spaces generalizes to paired nilpotent Lie algebras.}
...
Let $\cat C,\Fun T,z$, and $J\colon\pvect\to\cat C$ be as in \cref{prop.parameterized_representation}, with $\cat M=\pvect$. Suppose a lax symmetric monoidal functor
\[
...
\[
J(V)\otimes\Theta\widehat F(\lensob L)\to\Theta\widehat F(\lensob M).
\]
\end{corollary}
...
\begin{proposition}\label{prop.poly_potential_monad}
There is a monoidal monad on $\poly$
\[
\Fun T(p)\coloneqq p\otimes\cot\rr,
\]
whose unit and multiplication are $\cot 0\colon\yon\to\cot\rr$ and $\cot{+}\colon\cot\rr\otimes\cot\rr\to\cot\rr$.
\end{proposition}

\begin{proposition}\label{prop.cot_potential_lift}
The cotangent functor induces a strong symmetric monoidal functor
\[
\widehat\cot\colon\lmfd^\rr\to\Lens{\poly}^{\Fun T}.
\]
\end{proposition}
...
\]
The sign fixes the sign of Hamilton's equations below; $c=0$ gives the trivial algebra (no potential), $c=+1$ the opposite-sign physics. Other choices of value object $z\neq\yon$ are permitted by \cref{def.T_monoid}; we do not pursue them.

\begin{corollary}\label{cor.Phi1}
There is a lax symmetric monoidal functor
\[
...
A morphism $f\colon\lensob L\to\lensob M$ in $\potlens$ with parameter $(V,\sharp_V)$ gives the map
\begin{equation}\label{eqn.parameter_to_action}
\cot V\otimes\Phi_1\lensob L\to\Phi_1\lensob M.
\end{equation}

\iffalse
The $\pvect$-action on $\lmfd^\rr$ defined above and the action $V\cdot p\coloneqq\cot V\otimes p$ on $\poly$ fit into the parameterized representation of \cref{prop.parameterized_representation}: the comparison
\[
\cot V\otimes\Phi_1\lensob M\cong\Phi_1(V\cdot\lensob M)
\]
comes from the strong monoidality of $\cot$ on the output slot (\cref{prop.pnla_smc_action,prop.cot_monoidal}) and the comonoid structure of \cref{lem.cot_comonoid}. It gives
\[
...
The maps
\[
\theta_{V,p}\colon\cot{T^*V}\otimes p\To{\rho_V\otimes\id_p}\cot V\otimes p
\]
are an action square \eqref{eqn.para_square} from the action $V\cdot p=\cot V\otimes p$ to the action $V\cdot p=\cot{T^*V}\otimes p$. Applying \cref{prop.para_square} gives a strong symmetric monoidal functor
\[
\rho^*\colon\para{\pvect,\,\cot\circ\Fun{inc}}{\poly}\to\para{\pvect,\,\cot\circ\Fun{inc}\circ T^*}{\poly}.
\]
Then \cref{cor.cotangent_learners}'s coalgebra interpretation, applied at $T^*V$ (using \cref{prop.pnla_polynomial}: $\cot{T^*V}\cong (T^*V)\yon^{T^*V}$ since $T^*V$ is a paired vector space by \cref{prop.canonical_symplectic_pairing}), packages the result as an $\org$-morphism with state set $T^*V$. This is the 1-cell $\Phi(f)$ in $\org$.
