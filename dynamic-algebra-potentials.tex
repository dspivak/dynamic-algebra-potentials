...
\begin{corollary}[Parameter refinement]\label{cor.parameter_refinement}
Let $K,K'\colon\cat A\to\cat C$ be strong symmetric monoidal functors and let $\rho\colon K'\to K$ be a monoidal natural transformation. Writing $\para{\cat A,K}{\cat C}$ for the Para construction using the action $a\cdot x\coloneqq K(a)\otimes x$, there is a strong symmetric monoidal functor
\[
\rho^*\colon\para{\cat A,K}{\cat C}\to\para{\cat A,K'}{\cat C}
\]
which sends $(a,f\colon K(a)\otimes x\to y)$ to
\[
K'(a)\otimes x\To{\rho_a\otimes\id}K(a)\otimes x\To{f}y.
\]
\end{corollary}

\begin{proof}
The maps $\rho_a\otimes\id_x$ form an action square from the $K$-action to the $K'$-action. Applying \cref{prop.para_square} gives the functor, and monoidality of $\rho$ gives its strong symmetric monoidal structure.
\end{proof}
...
In this chapter we define a symmetric monoidal category $\potlens$ of potentialized lenses. \Cref{sec.lenses_general} develops lenses, their wiring-diagram operations, and the backward-monad construction. \Cref{sec.abstract_representation} packages the representation functors and their parameter refinements. \Cref{sec.potlens_manifolds} specializes the source-side constructions to manifolds: manifold lenses, the potential monad associated to any Lie monoid, and the PN Lie Algebra parameterization, synthesizing them as $\potlens$.
...
\section{The abstract representation}\label{sec.abstract_representation}

The previous section gives the category of lenses, and discusses the comonad and coKleisli category associated to a monad on the base category. 
...
This will be proved in \cref{prop.Theta_T_alpha}; indeed it corresponds to the case where $\Fun{T}=\id$ is the identity monad. We keep this basic case visible because it is more readable and interpretable. It says that we can interpret lens objects $\lensob{c}$as objects $\outp c\otimes\ihom{\inpt c,I}$ inside the base category.
...
Suppose $(\Fun{T},\tau)$ is a monoidal monad on a symmetric monoidal closed category $(\cat{C},I,\otimes, [-,-])$. Suppose further that the unit carries an algebra structure $\alpha\colon\Fun{T}I\to I$,
\[
\begin{tikzcd}[ampersand replacement=\&]
...
\end{tikzcd}
\]
For identities, the lens identity contributes the output counit, while the strength unit law, the algebra equation $\alpha\circ\eta_I=\id_I$, and the closure triangle identity reduce the display to the identity on $\outp c\otimes[\inpt c,I]$. For composition, expand the coKleisli composite. The two resulting composites agree by coassociativity and naturality of the output comonoids, the associativity and naturality axioms for the strength, functoriality of $\Fun T$, the closure triangle identity, and the algebra equation $\alpha\circ\mu_I=\alpha\circ\Fun T\alpha$.

The normal lax structure is $I\otimes I\cong I$ together with the canonical map
\[
(\outp c\otimes[\inpt c,I])\otimes(\outp d\otimes[\inpt d,I])
\to
\outp c\otimes\outp d\otimes[\inpt c\otimes\inpt d,I].
\]
Its compatibility with morphisms is exactly the monoidality of the monad and the assumption that $\alpha$ is a monoid homomorphism; normality uses $\alpha\circ\tau_0=\id_I$.
\end{proof}
...
Suppose $\cat A$ is symmetric monoidal, $\cat C$ is symmetric monoidal closed, $(\Fun T,\tau)$ is a monoidal monad on $\cat C$, and $\alpha\colon\Fun T I\to I$ is as in \cref{prop.Theta_T_alpha}. If $J\colon\cat A\to\cat C$ is strong monoidal and lands in commutative comonoids, then the $\cat A$-action
\[
a\cdot\lensob c\coloneqq\binom{\inpt c}{J(a)\otimes\outp c}
\]
on $\Lens{\cat C}^{\Fun T}$ and the action $a\cdot x\coloneqq J(a)\otimes x$ on $\cat C$ induce a lax symmetric monoidal functor
\[
\para{\cat A}{\Lens{\cat C}^{\Fun T}}\to\para{\cat A}{\cat C}.
\]
It sends a parameterized coKleisli lens $(a,f\colon a\cdot\lensob c\coto\lensob d)$ to the parameterized $\cat C$-morphism
\[
J(a)\otimes\Theta^{\Fun T,\alpha}\lensob c\to\Theta^{\Fun T,\alpha}\lensob d.
\]
\end{proposition}

\begin{proof}
The assumption on $J$ gives the action on lenses: the tensor comonoid supplies the output comonoid of $a\cdot\lensob c$, and the extra $J(a)$ in the output slot is killed in the backward leg by its counit. The natural comparison
\[
J(a)\otimes\Theta^{\Fun T,\alpha}\lensob c
\cong (J(a)\otimes\outp c)\otimes\ihom{\inpt c,I}
=\Theta^{\Fun T,\alpha}(a\cdot\lensob c)
\]
is compatible with action unitors and associators by coherence for the strong monoidal structure of $J$. Thus $\Theta^{\Fun T,\alpha}$ and the identity on $\cat A$ form an action square, and \cref{prop.para_square} gives the displayed functor. Its lax monoidal structure is inherited from \cref{prop.Theta_T_alpha}.
\end{proof}
...
\Cref{cor.parameter_refinement} applied to $K=\cot\circ\Fun{inc}$, $K'=\cot\circ\Fun{inc}\circ T^*$, and $\rho$ gives a functor comparing the actions $V\cdot p=\cot V\otimes p$ and $V\cdot p=\cot{T^*V}\otimes p$ on $\poly$. Then \cref{cor.cotangent_learners}'s coalgebra interpretation, applied at $T^*V$ (using \cref{prop.pnla_polynomial}: $\cot{T^*V}\cong (T^*V)\yon^{T^*V}$ since $T^*V$ is a paired vector space by \cref{prop.canonical_symplectic_pairing}), packages the result as an $\org$-morphism with state set $T^*V$. This is the 1-cell $\Phi(f)$ in $\org$.
...
\]
where $\Phi_1$ is from \cref{sec.specialize_source}, $\rho^*$ is the parameter refinement of \cref{cor.parameter_refinement} applied in \cref{sec.cotangent_state_refinement}, and $(*)$ is the coalgebra interpretation at state $T^*V$ (\cref{prop.coalg_as_poly_map,prop.pnla_polynomial}). Each is functorial; their composite is $\Phi$.

For lax monoidality: $\Phi_1$ is lax (composing strong $\widehat\cot$ with lax $\Theta^{\Fun T,\dec}$ from \cref{prop.Theta_T_alpha}), $\para\pvect{\Phi_1}$ inherits laxness, $\rho^*$ is strong monoidal by \cref{cor.parameter_refinement}, and $(*)$ is strong by \cref{prop.pnla_polynomial,prop.cot_monoidal}. Composing yields lax. The morphism laxator is the closedness pairing $\ihom{\cot{\inpt L},\yon}\otimes\ihom{\cot{\inpt{L'}},\yon}\to\ihom{\cot{\inpt L\times\inpt{L'}},\yon}$ of \cref{prop.Theta}, which is not invertible (it misses covector fields on $\inpt L\times\inpt{L'}$ with genuine mixed dependence), so $\Phi$ is lax, not strong.\qedhere
\end{proof}
