...
Suppose $\cat A,\cat A'$ are symmetric monoidal categories acting on $\cat D,\cat D'$, that $F\colon\cat A\to\cat A'$ is a strong monoidal functor, $G\colon\cat D\to\cat D'$ is a functor, and $\theta\colon F(a)\cdot G(x)\To{\cong}G(a\cdot x)$ is a natural isomorphism compatible with the action unitors and associators:
\begin{equation}\label{eqn.para_square}
\begin{tikzcd}[ampersand replacement=\&]
\cat A\times\cat D\ar[r,"(\cdot)"]\ar[d,"F\times G"']\ar[dr,phantom,"\cong^\theta"description]\& \cat D\ar[d,"G"]\\
...
Given strong monoidal functors $L,M,N,L'$ between symmetric monoidal categories and a monoidal isomorphism $\theta$ as on the left, one obtains an action square as on the right
\[
\begin{tikzcd}[ampersand replacement=\&]
\cat A\ar[r,"L"]\ar[d,"M"']\ar[dr,phantom,"\cong^\theta"description]\& \cat B\ar[d,"N"]\\
\cat A'\ar[r,"L'"']\& \cat B'
\end{tikzcd}
\quad\leadsto\quad
\begin{tikzcd}[ampersand replacement=\&]
\cat A\times\cat B\ar[r,"(\cdot)"]\ar[d,"M\times N"']\ar[dr,phantom,"\cong"description]\& \cat B\ar[d,"N"]\\
...
\]
Both factors are monoidal natural isomorphisms (the input $\theta$ and the strong monoidal comparison of $N$), so the composite is too; compatibility with action unitors and associators reduces to that of $\theta$ and the strong-monoidal data of $L,L',M,N$.\qedhere
\end{proof}

\begin{corollary}\label{cor.pvect_mfd_to_set_poly}
We have a strong symmetric monoidal functor
\[
\para{\pvect}{\mfd}\;\longrightarrow\;\para{\smset_\cong}{\poly}.
\]
\end{corollary}
\begin{proof}
Apply \cref{prop.para_square,lem.functor_square_action} to \cref{prop.pnla_polynomial}. The functor sends $M\mapsto\cot{M}$ and $f\colon V\times M\to N$ to the morphism with parameter $|V|$ and map
\[
V\yon^V\otimes\cot{M}\;\To{\theta_V\otimes\id}\;\cot{V}\otimes\cot{M}\;\To{\cong}\;\cot{(V\times M)}\;\To{\cot f}\;\cot{N},
\]
where the second arrow is the strong monoidal comparison of $\cot$ (\cref{prop.cot_monoidal}).\qedhere
\end{proof}
...
\subsection{Cotangent learners}\label{sec.cotangent_learners}

Composing \cref{cor.pvect_mfd_to_set_poly} with \cref{prop.org_as_para} gives a strong symmetric monoidal functor
\begin{equation}\label{eqn.cotangent_learners}
\Psi\colon\para{\pvect}{\mfd}\to\org,
\end{equation}
the \emph{cotangent-learners} functor. On objects, $M\mapsto\cot M$. On a morphism $(V,f\colon V\times M\to N)$, $\Psi(V,f)$ is the $\ihom{\cot M,\cot N}$-coalgebra with state set $V$ whose action at $v\in V$ is $\cot f_v\colon\cot M\to\cot N$ (with $f_v(m)\coloneqq f(v,m)$) and whose update on a direction $(m,\nu)$ with $\nu\in T^*_{f(v,m)}N$ is
...
The construction above uses the monoidal unit $z\coloneqq I$ as the target of evaluation. More generally, for any $\otimes$-monoid $(z,e_z,m_z)$ equipped with a $\Fun{T}$-algebra $\alpha\colon\Fun{T}z\to z$ which is also a monoid homomorphism, the above generalize to 
\[
...
We apply the Para construction (\cref{sec.para_general}) to lenses. Let $J\colon\cat{A}\to\cat{C}$ be a strong monoidal functor whose values are commutative comonoids and whose maps and monoidal structure maps preserve those comonoids. It gives an action of $\cat{A}$ on $\Lens{\cat{C}}^\Fun{T}$ by
\[
...
\[
P\times\outp{M}\to\outp{N}
\qqand
P\times\outp{M}\times\inpt{N}\to\inpt{M}\times\rr
\]
...
We lift the $\pvect$-action on $\mfd$ from \cref{prop.pnla_smc_action} to $\lmfd$ by acting on the $\outp M$-slot:
\[
(\cdot)\colon\pvect\times\lmfd\to\lmfd,\qquad V\cdot\lensob{M}\coloneqq\binom{\inpt M}{V\times\outp M}.
\]
(One could equally well act on the $\inpt M$-slot; we choose the $\outp M$-slot because its product with $V$ automatically inherits a comonoid structure, as required by the definition of a lens.)
...
\]
Unfolding, a morphism $f\colon\lensob{L}\to\lensob{M}$ in $\potlens$ is a tuple $f=(V,\outp f,\inpt f,U)$ with $V$ a paired vector space,
...
\]
where $\pvect$ acts on $\poly$ by
\[
V\cdot p\coloneqq\cot{V}\otimes p.
\]
Thus a morphism $f\colon\lensob{L}\to\lensob{M}$ in $\potlens$ with parameter PN Lie Algebra $(V,\sharp_V)$ gives a polynomial map
\[
...
Applying \cref{prop.pnla_polynomial} to the paired vector space $(V,\sharp_V)$ gives
\[
\cot{V}\cong V\yon^V.
\]
By \cref{prop.coalg_as_poly_map}, the map \eqref{eqn.parameter_to_action} is equivalently an $\ihom{\Phi_1\lensob{L},\Phi_1\lensob{M}}$-coalgebra with state set $V$. This is the $\org$-morphism obtained from the parameterized lens before adding the phase-space refinement.

\section{The cotangent-state refinement}\label{sec.cotangent_state_refinement}

For the dynamics considered in this paper, we use $T^*V$ rather than $V$ as the state space. This is an additional step, not part of the abstract construction above. It fits a general framework: choose a strong monoidal endofunctor $F\colon\pvect\to\pvect$ and a monoidal natural transformation $\rho\colon\cot\circ F\Rightarrow\cot$, and precompose.

\begin{proposition}[Parameter-refinement by a monoidal natural transformation]\label{prop.param_refinement}
Let $F\colon\pvect\to\pvect$ be a strong monoidal endofunctor and let $\rho\colon\cot\circ F\Rightarrow\cot$ be a monoidal natural transformation (naturality required only for $\pvect$-isomorphisms, since $\pvect$ is a groupoid). Precomposition with $\rho$ defines an identity-on-objects functor
\[
\rho^*\colon\Para_{\pvect,\,\cot}(\poly)\;\longrightarrow\;\Para_{\pvect,\,\cot\circ F}(\poly),
\qquad
(V,\,f\colon\cot{V}\otimes p\to q)\;\mapsto\;\bigl(V,\,f\circ(\rho_V\otimes\id)\bigr).
\]
Composing with the coalgebra interpretation from \cref{prop.pnla_polynomial,prop.coalg_as_poly_map}, applied to $F(V)$, yields a functor $\Para_{\pvect,\,\cot}(\poly)\to\org$ with state set $F(V)$.
\end{proposition}

\begin{proof}
$\rho^*$ is identity on objects, sending a morphism $(V,f\colon\cot{V}\otimes p\to q)$ to $(V,\,f\circ(\rho_V\otimes\id))\colon\cot{F(V)}\otimes p\to q$.

\emph{Identity preservation.} The Para identity on $p$ has parameter $\rr^0$ (the unit of $\pvect$) and morphism the action unitor $\cot{\rr^0}\otimes p\cong p$. Strong monoidality of $F$ gives $F(\rr^0)\cong\rr^0$, and monoidality of $\rho$ identifies $\rho_{\rr^0}$ with the canonical $\cot{F(\rr^0)}\cong\cot{\rr^0}$. Hence $\rho^*(\id_p)$ equals the action unitor for $\cot\circ F$, i.e., the identity in $\Para_{\pvect,\cot\circ F}(\poly)$.

\emph{Composition preservation.} For composable Para morphisms $(V,f)\colon p\to q$ and $(W,g)\colon q\to r$, the composite $(W\oplus V,h)$ has
\[
h\colon\cot{W\oplus V}\otimes p\To{\sigma\otimes\id}\cot{W}\otimes\cot{V}\otimes p\To{\id\otimes f}\cot{W}\otimes q\To{g}r,
\]
where $\sigma\colon\cot{W\oplus V}\To{\cong}\cot{W}\otimes\cot{V}$ is the strong-monoidal comparison from \cref{prop.cot_monoidal}. The Para composite of $\rho^*(V,f)$ and $\rho^*(W,g)$ in $\Para_{\pvect,\cot\circ F}(\poly)$ uses the analogous comparison $\sigma_F\colon\cot{F(W\oplus V)}\To{\cong}\cot{F(W)}\otimes\cot{F(V)}$ for the action $V\mapsto\cot{F(V)}$ and reads
\[
\cot{F(W\oplus V)}\otimes p\To{\sigma_F\otimes\id}\cot{F(W)}\otimes\cot{F(V)}\otimes p\To{(\rho_W\otimes\rho_V)\otimes\id}\cot{W}\otimes\cot{V}\otimes p\To{\id\otimes f}\cot{W}\otimes q\To{g}r.
\]
Monoidality of $\rho$ asserts that the square
\[
\begin{tikzcd}[ampersand replacement=\&]
\cot{F(W\oplus V)}\ar[r,"\rho_{W\oplus V}"]\ar[d,"\sigma_F"']\&\cot{W\oplus V}\ar[d,"\sigma"]\\
\cot{F(W)}\otimes\cot{F(V)}\ar[r,"\rho_W\otimes\rho_V"']\&\cot{W}\otimes\cot{V}
\end{tikzcd}
\]
commutes; this identifies $\rho^*(W\oplus V,h) = (W\oplus V,h\circ(\rho_{W\oplus V}\otimes\id))$ with the Para composite of $\rho^*(V,f)$ and $\rho^*(W,g)$.

\emph{Coalgebra interpretation.} For a morphism $(V,h\colon\cot{F(V)}\otimes p\to q)$ in $\Para_{\pvect,\cot\circ F}(\poly)$, applying \cref{prop.pnla_polynomial} to the paired vector space $F(V)\in\pvect$ gives $\cot{F(V)}\cong F(V)\yon^{F(V)}$, so $h$ is a polynomial map $F(V)\yon^{F(V)}\otimes p\to q$. By \cref{prop.coalg_as_poly_map} this presents $h$ as an $\ihom{p,q}$-coalgebra with state set $F(V)$, equivalently a 1-cell in $\org$. Functoriality is direct: composition of two such morphisms uses $\cot{F(W\oplus V)}\cong\cot{F(W)}\otimes\cot{F(V)}$ together with \cref{prop.pnla_polynomial}, which corresponds to coalgebra composition in $\org$.\qedhere
\end{proof}
...
For any paired vector space $(V,\sharp_V)$, \eqref{eqn.tangent_vec} gives
\[
T^*V\cong V\oplus V^*.
\]
We call a point $(u,\pi)$ a pair consisting of a \emph{position} $u\in V$ and a \emph{momentum} $\pi\in V^*$. The cotangent space there decomposes as
\[
T^*_{(u,\pi)}(T^*V)\cong T^*_uV\oplus T^*_\pi(V^*)\cong V^*\oplus V,
\]
both identifications by \eqref{eqn.tangent_vec}, using $(V^*)^*\cong V$ on the $V^*$-factor.
...
On positions, $\rho_V$ forgets momentum; on directions, it passes the incoming covector $\sigma$ through to the $V^*$-slot and injects the pairing-lifted momentum $\sharp_V(\pi)\in V$ into the $V$-slot, independently of $\sigma$. Thus $\rho_V$ is not of the form $\cot{h}$ for any smooth $h\colon T^*V\to V$: the $V$-component of its direction-action is constant in $\sigma$, whereas $\cot$ always produces a linear direction-action.

Precomposing \eqref{eqn.parameter_to_action} with $\rho_V$ gives
\[
\cot{T^*V}
\To{\rho_V}
\cot{V}
...
\ihom{\Phi_1\lensob{L},\Phi_1\lensob{M}}.
\]
Since $T^*V$ is a paired vector space by \cref{prop.canonical_symplectic_pairing}, \cref{prop.pnla_polynomial} gives
\[
\cot{T^*V}\cong (T^*V)\yon^{T^*V}.
\]
Thus \cref{prop.coalg_as_poly_map} presents the above map as an $\ihom{\Phi_1\lensob{L},\Phi_1\lensob{M}}$-coalgebra with state set $T^*V$. This is the 1-cell $\Phi(f)$ in $\org$.

\begin{remark}[Tangent-state alternative]\label{rmk.tangent_state}
Taking $F\coloneqq T$ (\cref{prop.TT_endofunctors}) instead of $T^*$ fits \cref{prop.param_refinement} equally well. The analogous \emph{tangent projection} $\tilde\rho_V\colon\cot{TV}\to\cot{V}$ has position-action $(u,X)\mapsto u$ and direction-action $\sigma\mapsto(\sigma,\,\flat_V(X))$ under the decomposition $T^*_{(u,X)}(TV)\cong V^*\oplus V^*$ (both factors via \eqref{eqn.tangent_vec}), using the flat map $\flat_V\colon V\to V^*$ to place $X$ in the second summand. This produces a functor $\potlens\to\org$ with state set $TV$, giving a Lagrangian-style (second-order) state update instead of Hamilton's equations. Beyond existence, we do not develop this alternative in the paper. The category-theoretic framework of \cref{prop.param_refinement} does not, by itself, single out $T^*$ over $T$; the preference for $T^*$ here is physics (cotangent bundles carry the canonical symplectic pairing of \cref{prop.canonical_symplectic_pairing}, and the resulting dynamics coincide with textbook Hamilton's equations).
\end{remark}
...
\]
are naturally isomorphic: on each morphism $f\colon L\to M$, the 2-cell in $\org$ induced by $\sharp_V\colon T^*V\to TV$ identifies $\Phi^{T^*}(f)$ with $\Phi^{T}(f)$ as 1-cells. This is the \emph{Legendre equivalence} of Hamiltonian and Lagrangian mechanics in our setting: the two formulations disagree on literal state spaces ($T^*V$ vs $TV$), but the pairing mediates a canonical isomorphism between them. The paper works with $\Phi^{T^*}$ for the reasons given in \cref{rmk.tangent_state}.
\end{remark}
...
\To{(*)}\org,
\]
where $\Phi_1\colon\lmfd^\rr\to\poly$ is the composite of the cotangent lift $\widehat\cot\colon\lmfd^\rr\to\Lens\poly^{\Fun T}$ (induced from \cref{prop.cot_monoidal} by lensification, with the writer monad on $\rr$ transported via $\cot{0},\cot{+}$ from \cref{prop.cot_hopf}) with $\Theta^{\Fun T,\dec}\colon\Lens\poly^{\Fun T}\to\poly$ (\cref{prop.Theta_T_alpha}); $\rho^*$ is the parameter refinement of \cref{prop.param_refinement} for $F\coloneqq T^*$ and $\rho$ the pairing projection of \cref{def.pairing_proj}; and $(*)$ is the coalgebra interpretation $(V,f)\mapsto$ ($\ihom{p,q}$-coalgebra with state $T^*V$) from \cref{prop.pnla_polynomial} and \cref{prop.coalg_as_poly_map}, applied at $T^*V$.

Each step is a functor: $\Phi_1$ as a composite of functors; $\para{\pvect}{\Phi_1}$ as the standard Para lift of a functor that preserves the actions; $\rho^*$ by the first half of \cref{prop.param_refinement}; and $(*)$ by the second half. Their composite is $\Phi$.\qedhere
\end{proof}

\section{Lax symmetric monoidality}\label{sec.lax_monoidal}

\begin{proposition}\label{prop.potlens_monoidal}
$\potlens$ inherits from $\lmfd$ and $\pvect$ a symmetric monoidal structure with unit $\binom{\rr^0}{\rr^0}$ and tensor product
\[
\lensob L\otimes\lensob{L'}\coloneqq\lensobs{L}{L'}.
\]
On morphisms, $f\otimes f'$ has parameter $V\times V'$ (with pairing $\sharp_V\oplus\sharp_{V'}$), independent forward and backward components, and potential $U^\otimes=U\circ\pr+U'\circ\pr'$.
\end{proposition}

\begin{proof}
$\potlens=\para{\pvect}{\lmfd^\rr}$. The Para construction over a symmetric monoidal action of an SMC on an SMC is itself symmetric monoidal: unit equals the unit of the underlying category (acted on by the unit of the action), tensor is parameterwise and slotwise on objects, and tensor of morphisms uses the strong-monoidal comparison of the action $(a\otimes b)\cdot(x\otimes z)\cong(a\cdot x)\otimes(b\cdot z)$ to interleave parameters; coherence is inherited componentwise from $\cat A$, $\cat D$, and the action.

Here $\pvect$ is SMC by \cref{prop.pnla_monoidal}, and $\lmfd^\rr=\Lens\mfd^\rr$ inherits an SMC structure from $(\mfd,\times)$ via lensification together with the writer monad on $\rr$, whose monoid structure $0\colon\rr^0\to\rr$ and $+\colon\rr\times\rr\to\rr$ make $\rr$ a symmetric monoid object in $\mfd$. The $\pvect$-action on $\lmfd^\rr$ factors through the strong monoidal forgetful $\pvect\to\mfd$ (\cref{prop.pnla_smc_action}) and is symmetric monoidal. Reading off the inherited structure: the unit is $\binom{\rr^0}{\rr^0}$, and the tensor of $(V,\lensob L)$ with $(V',\lensob{L'})$ is $(V\oplus V',\lensobs{L}{L'})$, with independent forward $\outp f\times\outp{f'}$, independent backward $\inpt f\times\inpt{f'}$, and additive potential $U^\otimes=U\circ\pr+U'\circ\pr'$ from the writer's monoid structure.\qedhere
\end{proof}

\begin{theorem}\label{thm.lax_monoidal}
$\Phi\colon\potlens\to\org$ is lax symmetric monoidal. The morphism laxator $\Phi(f)\otimes\Phi(f')\to\Phi(f\otimes f')$ has state component the canonical $T^*V\times T^*V'\cong T^*(V\times V')$, and its interface component is the tensor of \cref{prop.cot_monoidal}'s strong comparison on the $\cot{\outp{(-)}}$ factor with the pairing
\[
...
\item $\para{\pvect}{\Phi_1}$ is lax symmetric monoidal: the Para lift of a (lax/strong) symmetric monoidal action-preserving functor is (lax/strong) of the same strength, since the laxator and unit comparison transport componentwise through the strong-monoidal action interchange isomorphism.
\item $\rho^*$ is symmetric monoidal: the proof of \cref{prop.param_refinement} establishes that the Para tensor is preserved by $\rho^*$, with the necessary commutative square supplied by monoidality of $\rho$. Here the pairing projection $\rho$ is monoidal because $\sharp_{V_1\oplus V_2}=\sharp_{V_1}\oplus\sharp_{V_2}$ (\cref{prop.TT_monoidal}) and $T^*(V_1\oplus V_2)\cong T^*V_1\oplus T^*V_2$ (\cref{prop.TT_monoidal} again).
\item The coalgebra interpretation $(*)$ is strong symmetric monoidal: \cref{prop.pnla_polynomial} gives $\cot{V}\cong V\yon^V$, and the strong-monoidal comparison $\cot{V\oplus W}\cong\cot{V}\otimes\cot{W}$ from \cref{prop.cot_monoidal} corresponds to the polynomial-tensor comparison $(V\yon^V)\otimes(W\yon^W)\cong(V\times W)\yon^{V\times W}$, which is the comparison used to define the SMC structure on $\org$ (\cref{prop.org}).
\end{itemize}
Composing strong $\circ$ monoidal $\circ$ lax $\circ$ strong yields a lax symmetric monoidal composite. Only $\Theta^{\Fun T,\dec}$ contributes nontrivial laxness; its forward laxator is the closedness pairing $\ihom{m,I}\otimes\ihom{n,I}\to\ihom{m\otimes n,I}$ of \cref{prop.Theta} (combined with the writer's $\tau_{I,I}$ and algebra $\dec$). Pulled through the strong identifications, this gives the morphism laxator stated in the theorem: state component $T^*V\otimes T^*V'\cong T^*(V\oplus V')$ from \cref{prop.TT_monoidal}; cotangent-bundle component $\cot{\outp L}\otimes\cot{\outp{L'}}\cong\cot{\outp L\times\outp{L'}}$ from \cref{prop.cot_monoidal}; hom-factor component $\ihom{\cot{\inpt L},\yon}\otimes\ihom{\cot{\inpt{L'}},\yon}\to\ihom{\cot{\inpt L\times\inpt{L'}},\yon}$ from \cref{prop.Theta}. Coherence (associator, unitor, symmetry) is inherited from $(\mfd,\times)$ and $(\poly,\otimes)$ via the strong comparisons. The hom pairing is not invertible (\cref{prop.Theta} is lax, not strong), so $\Phi$ is lax, not strong.\qedhere
\end{proof}
...
\]
obtained by inserting the strength $\sigma$ into the $\chi_f^{\Fun{T},\alpha}$ construction of \cref{prop.Theta_T_alpha} at variable $x$ in place of $I$. Under the monoidal hypotheses on $\Fun{T}$, $\Fun{B}^{\Fun{T}}$ is strong monoidal, with Day-convolution comparison induced by $\tau_{x,y}\colon\Fun{T}x\otimes\Fun{T}y\to\Fun{T}(x\otimes y)$.
\end{proposition}
