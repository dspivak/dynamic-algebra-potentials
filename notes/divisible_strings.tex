...
\trackTerm{\Open}{opencat}{\Cat{Open}}% semicategory of open harmonic systems (1-categorical: \Cat, not \mathbb)
\trackTerm{\Lin}{lin}{\mathbb{L}\Cat{in}}% semicategory of ported machines
\trackTerm{\Ufun}{Ufun}{\mathsf{U}}% embedding of ported machines into PC
\trackTerm{\Phiopen}{Phiopen}{\Phi_{\mathrm{open}}}% open linear semantics
\trackTerm{\seam}{seam}{\fun{seam}}% seam wiring
\trackTerm{\Qu}{Qu}{Q}% queue state space Q_N(B)
\trackTerm{\Qterm}{Qterm}{\mathbb{Q}}% terminal delay realization
\trackTerm{\Fl}{Fl}{\Fn{Fl}}% covector fields on a Hilbert space
\trackTerm{\blk}{blk}{\fun{blk}}% block steps as a natural transformation
\trackTerm{\headf}{head}{\fun{head}}% completed-step head
\trackTerm{\nextf}{next}{\fun{next}}% completed-step next
\trackTerm{\Zport}{Zport}{P_Z}% the Z-port
\trackTerm{\cellm}{cellm}{\fun{cell}}% the cell
\trackTerm{\Chain}{chain}{\fun{chain}}% chains = powers of the cell
\trackTerm{\Tm}{Tm}{\mathsf{T}}% the chain machine
\trackTerm{\Sm}{Sm}{\mathsf{S}}% the string
...
The semicategory $\defineTerm{opencat}$ has as objects the ports.  A
...
\EZ(a)\oplus \EZ(d).
\end{equation}
We write $\defineTerm{head}_{d,a}\colon\EZ(d)\to\EZ(a)$ and
$\defineTerm{next}_{d,a}\colon\EZ(d)\oplus\EZ(a)\to\EZ(d)$ for its two
components.
\end{definition}
